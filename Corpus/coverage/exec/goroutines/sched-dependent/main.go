...
// review): with BOTH senders parked on the ONE unbuffered channel when
// main's receive arrives, the arrival pairing must choose among TWO
// matching parked waiters — the only corpus shape whose certified tree
// consumes the L4 pick above width 1 (first-come's senders commit to a
// cap-2 buffer and never park; select-arrival-multi's parked senders
// sit on different channels, so the choice there is the L2 clause
// pick). The receive order {12, 21} is generated by L1 arrival timing
