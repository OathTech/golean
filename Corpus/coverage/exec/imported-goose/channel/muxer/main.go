...
// --- GoLean harness ---
// Authored wrappers: each entry point's greeting string is the
// observable (rendezvous channels serialize every request/response).

func goleanAsync() string {
	return <-Async(func() string { return "as" + "ync" })
}

func goleanClient() string {
	return Client()
}

func goleanClientOld() string {
	return ClientOld()
}

func goleanMakeGreeting() string {
	return makeGreeting()
}
