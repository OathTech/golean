...
/-! # A4-U22 (C2d): THE FULL-ROUND GENERATOR — the MsgApp
append-family round, anchor to anchor (23,488 steps, 8 draws: the
SEMANTIC delivery pick @207 + 7 latitude appendSpills), mirrored in
12 windows with GENERALIZED DIFF-CROSSINGS (the tree-propagation
template: crossing post-states are set/append-diff expressions over
the pre-states; spill post-configs share the pre's continuation;
the pick post-config is reflected + γ-checked). Prints RoundMaLit
literals for the canonical run behind the R-form's first instance. -/

namespace RoundGen
open GoLean GoLean.GoCore GoLean.GoCore.Machine GoLean.Sym GoLean.RaftSeam

/-- Round-relative boundary schedule: windows split at the 8 crossing
indexes (census `msgappring.out` + fixture probe) + 3 balance splits. -/
def rSched : List (Nat × String) := [
  (0, "R0"),
  (207, "RP1a"), (208, "RP1b"),            -- the PICK (semantic)
  (3000, "R1"),
  (5837, "RP2a"), (5838, "RP2b"),          -- arm spill 1
  (7212, "RP3a"), (7213, "RP3b"),          -- arm spill 2
  (10682, "RP4a"), (10683, "RP4b"),        -- ring X1
  (12610, "RP5a"), (12611, "RP5b"),        -- ring X2
  (15383, "RP6a"), (15384, "RP6b"),        -- ring X3 (ents)
  (15746, "RP7a"), (15747, "RP7b"),        -- ring X4 (net)
  (15796, "RP8a"), (15797, "RP8b"),        -- ring X5 (live)
  (18500, "R2"), (21295, "R3"), (23488, "R4")]

def rSegs : List (String × Nat × String) := [
  ("R0", 207, "RP1a"), ("RP1b", 2792, "R1"), ("R1", 2837, "RP2a"),
  ("RP2b", 1374, "RP3a"), ("RP3b", 3469, "RP4a"), ("RP4b", 1927, "RP5a"),
  ("RP5b", 2772, "RP6a"), ("RP6b", 362, "RP7a"), ("RP7b", 49, "RP8a"),
  ("RP8b", 2703, "R2"), ("R2", 2795, "R3"), ("R3", 2193, "R4")]

partial def walkCollectR (n : Nat) (σ : ExecState) (c : Config) (ch : Choices)
    (stopAt : Nat) (acc : List (String × ExecState × Config)) :
    IO (List (String × ExecState × Config)) := do
  let acc ← match rSched.find? (fun (i, _) => i == n) with
    | some (_, name) => do
        IO.println s!"  @[{n}] {name}: na={σ.nextAddr} heap={σ.heap.length} ch={ch.length} :: {RingGen.cfgHead c}"
        pure ((name, σ, c) :: acc)
    | none => pure acc
  if n == stopAt then return acc.reverse
  match stepFn σ c ch with
  | .ok (c2, σ2, ch2) => walkCollectR (n+1) σ2 c2 ch2 stopAt acc
  | .error e => IO.println s!"WALK ERROR @{n}: {e.message.take 150}"; return acc.reverse

/-- Positional heap diff (machine level): changed (idx, loc, postCell)
and appended (loc, postCell). Fail-noisy on loc moves. -/
...
/-- Apply a reflected diff to a mirror state (set changed; append new;
post na). -/
...
/-- Print the diff expression for a crossing post state over `preName`. -/
def pDiffState (preName : String) (changed appended : List (Loc × GoCore.HeapCell))
    (naPost : Nat) : String :=
  let setPart := changed.foldl (fun acc (lc : Loc × GoCore.HeapCell) =>
    s!"(GoLean.Sym.Heap.set {acc} {BfLitGen.pr lc.1} {BfLitGen.pCell (reflectCell symDom lc.2)})")
    s!"maS{preName}.heap"
  let appPart := "[" ++ String.intercalate ",\n    " (appended.map (fun (lc : Loc × GoCore.HeapCell) =>
    s!"({BfLitGen.pr lc.1}, {BfLitGen.pCell (reflectCell symDom lc.2)})")) ++ "]"
  s!"\{ heap := ({setPart} ++ {appPart}),\n  nextAddr := {naPost} }"

end RoundGen

open BfLitGen RingGen RoundGen GoLean GoLean.GoCore GoLean.GoCore.Machine GoLean.Sym GoLean.Surface GoLean.RaftSeam in
#eval show IO Unit from do
  ...
  -- 1. concrete boundary collection over the whole round
  let states ← RoundGen.walkCollectR 0 σP cA chA 23488 []
  if states.length != RoundGen.rSched.length then
    IO.println s!"COLLECT MISMATCH: {states.length} of {RoundGen.rSched.length}"; return
  let get := fun (name : String) =>
    (states.find? (fun (n, _, _) => n == name)).getD ("MISSING", σP, cA)
  -- 2. mirror propagation with diff-crossings + γ-validation
  let mut mstates : List (String × SymState × SymConfig) :=
    [("R0", RingGen.reflectS σP, reflectC symDom cA)]
  -- printed def bodies for crossing posts (name → body string); windows print pState
  let mut exprDefs : List (String × String × String) := []  -- (name, stateBody, cfgBody)
  let mget := fun (ms : List (String × SymState × SymConfig)) (name : String) =>
    (ms.find? (fun (n, _, _) => n == name)).getD
      ("MISSING", RingGen.reflectS σP, reflectC symDom cA)
  let mut allOk := true
  for (a, len, b) in RoundGen.rSegs do
    ...
    if b.startsWith "RP" && b.endsWith "a" then
      -- crossing: machine pre = σb; machine post = the (b-dropLast+"b") state
      let postName := String.ofList b.toList.dropLast ++ "b"
      let (_, σpost, cpost) := get postName
      let (changed, appended) ← RoundGen.heapDiff σb.heap σpost.heap
      IO.println s!"    cross {b}->{postName}: changed={changed.length} appended={appended.length} :: {RingGen.cfgHead cb}"
      let Spost := RoundGen.applyDiff S' changed appended σpost.nextAddr
      -- post config: spill => shared .next extract; pick => reflect
      let (Cpost, cfgBody) := match C' with
        | .retV _ (.stmtOpK _ _ _ _ _ k') =>
            ((.next k' : SymConfig),
             s!"Config.next (match maC{b} with\n  | .retV _ (.stmtOpK _ _ _ _ _ k) => k\n  | _ => .stop)")
        | _ => (reflectC symDom cpost, BfLitGen.pC (reflectC symDom cpost))
      -- γ-check the constructed post
      let σγp := γS ρ0 σP Spost
      let hEqP := σγp.heap == σpost.heap
      let naEqP := σγp.nextAddr == σpost.nextAddr
      let cEqP := toString (repr (γC ρ0 Cpost)) == toString (repr cpost)
      IO.println s!"    post γheap={hEqP} γna={naEqP} γcfg={cEqP}"
      if !hEqP || !naEqP || !cEqP then allOk := false
      mstates := (postName, Spost, Cpost) :: (b, S', C') :: mstates
      exprDefs := (postName,
        RoundGen.pDiffState b changed appended σpost.nextAddr, cfgBody) :: exprDefs
    else
      mstates := (b, S', C') :: mstates
  if !allOk then IO.println "PROPAGATION FAILED — not printing"; return
  IO.println "round mirror propagation γ-valid at all boundaries + crossings"
  -- 3. print
  let hdrCommon := "import GoLeanProofs.Sym.Mirror\n\n/-! GENERATED (A4-U22 C2d; `artifacts/probe/RoundMaGen.lean` — DO NOT\nEDIT BY HAND). The MsgApp append-family FULL-ROUND literals (anchor\nto anchor, 23,488 steps, 8 draws), tree-propagated; crossing posts\nare diff-expressions over their pre-states (the C2c template). The\nwindow LINK theorems in `RoundMaEq*.lean` re-check every literal\nagainst the mirror. -/\n\nnamespace GoLean.RaftSeam.RoundMa\n\nopen GoLean GoLean.GoCore GoLean.Sym\n\nset_option maxRecDepth 8000000\n\n"
  let names := RoundGen.rSched.map (fun (_, n) => n)
  ...
  out := out ++ "end GoLean.RaftSeam.RoundMa\n"
  IO.FS.writeFile "../artifacts/probe/gen/RoundMaLit.lean.gen" out
  IO.println s!"written RoundMaLit.lean.gen: {out.length} chars"
  let (_, σ4, _) := get "R4"
  IO.println s!"R4: na={σ4.nextAddr} heap={σ4.heap.length}"
