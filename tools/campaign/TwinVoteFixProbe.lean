...
/-- The doctored MsgVote, THE REAL VOTE FAMILY (budget item): Type 5
(na = 5), To 2, From 1, Term → na+6 (1 — a REAL higher term: node 2
is at Term 0, so Step takes the m.Term > r.Term branch →
becomeFollower(1, None) → the MsgVote arm grants on the up-to-date
check against the boot log (LogTerm → na+11 (1), Index → na+12 (1) =
the (1,1) last entry)), no entries, Commit nil. -/
