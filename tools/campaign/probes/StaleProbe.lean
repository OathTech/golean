...
the STALE branch
(m.Index = 0 < committed = 1, m.LogTerm = term(1) = 1, m.Commit =
...
def staleHeap : GoCore.Heap :=
  ...
  [(.base ⟨52⟩, ⟨some (.defined ⟨"raftpb.Message"⟩), staleMsgVal⟩),
   ...
   (.base ⟨56⟩, ⟨some (.int .uint64), .int 0 .uint64⟩)]

def staleσ0 : ExecState := { wBase with heap := staleHeap, nextAddr := 57 }
...
#eval match stepFnIter 2798 staleσ0 staleC0 bigStream with
