...
/-! # A4-U24: THE MSGAPPRESP MAYBECOMMIT FULL-ROUND GENERATOR — the real vote family
...
/-- TwinMarFixProbe's printed read set (`marfix.out`). -/
def keptCells : List Nat :=
  [15, 23, 25, 27, 28, 57, 110, 121, 170, 179, 295, 298, 349, 389,
   1086, 1103, 1107, 1110, 1212, 1742, 1764, 1770, 3369, 5666, 5669,
   5672, 5675, 5758, 6070, 6072, 8520, 8639, 8642, 8644, 9384, 9601,
...
#eval show IO Unit from do
  ...
  | .ok (c₀, s₃, _, ch₁) =>
  match VoteGen.walkToAnchorN 3 s₃ c₀ ch₁ with
  ...
  for (nm, Sx, Cx) in ordered do
    out := out ++ BfLitGen.emitDef s!"mrS{nm}" "SymState" (BfLitGen.pState Sx)
    out := out ++ BfLitGen.emitDef s!"mrC{nm}" "SymConfig" (BfLitGen.pC Cx)
    inFile := inFile + 1
    if inFile ≥ 5 then
      out := out ++ "end GoLean.RaftSeam.RoundMar\n"
      IO.FS.writeFile s!"../artifacts/probe/gen/RoundMarLit{fileNo}.lean.gen" out
      IO.println s!"written RoundMarLit{fileNo}.lean.gen: {out.length} chars"
      out := hdrCommon
      fileNo := fileNo + 1
      inFile := 0
  if inFile > 0 then
