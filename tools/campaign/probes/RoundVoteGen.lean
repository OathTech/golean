...
/-! # A4-U23: THE MSGVOTE FULL-ROUND GENERATOR — the real vote family
...
/-- The doctored MsgVote, THE REAL VOTE FAMILY (TwinVoteFixProbe's,
verbatim): Type 5, To 2, From 1, Term na+6 (1 — a REAL higher term),
LogTerm na+11 (1) / Index na+12 (1) = the (1,1) last entry of the
decoded boot log, no entries, Commit nil. -/
...
def doctor (σ : ExecState) : Option ExecState := do
  ...
  let heap' := GoCore.Heap.set σ.heap tl ⟨tc.declaredTy, tv'⟩ ++
    [(.base ⟨na⟩, ⟨some (.int .int32), .int 5 .int32⟩),
...
/-- TwinVoteFixProbe's printed 44-cell read set (`votefix.out`). -/
def keptCells : List Nat :=
  [15, 18, 27, 28, 57, 67, 110, 121, 170, 1764, 1770, 1779, 1886, 1895,
   1898, 1900, 1949, 1989, 2703, 2707, 2710, 2812, 3199, 3342, 3344,
   3351, 3354, 3360, 3369, 6070, 6072, 6073, 6074, 6075, 6076, 6077,
...
/-- Round-relative boundary schedule: windows split at the 9 crossing
indexes (census `votering.out`) + 4 balance splits. -/
def vSched : List (Nat × String) := [
  (0, "V0"),
  (207, "VP1a"), (208, "VP1b"),            -- the PICK (semantic)
  (3522, "VP2a"), (3523, "VP2b"),          -- becomeFollower mapIter 1
  (3706, "VP3a"), (3707, "VP3b"),          -- becomeFollower mapIter 2
  (3735, "VP4a"), (3736, "VP4b"),          -- becomeFollower mapIter 3
  (3764, "VP5a"), (3765, "VP5b"),          -- becomeFollower mapIter 4
  (6100, "V1"), (8400, "V2"),
  (10764, "VP6a"), (10765, "VP6b"),        -- send-assembly spill
  (13408, "VP7a"), (13409, "VP7b"),        -- Ready-assembly spill
  (15193, "VP8a"), (15194, "VP8b"),        -- accept spill 1
  (15243, "VP9a"), (15244, "VP9b"),        -- accept spill 2
  (17200, "V3"), (19291, "V4")]

def vSegs : List (String × Nat × String) := [
  ("V0", 207, "VP1a"), ("VP1b", 3314, "VP2a"), ("VP2b", 183, "VP3a"),
  ("VP3b", 28, "VP4a"), ("VP4b", 28, "VP5a"), ("VP5b", 2335, "V1"),
  ("V1", 2300, "V2"), ("V2", 2364, "VP6a"), ("VP6b", 2643, "VP7a"),
  ("VP7b", 1784, "VP8a"), ("VP8b", 49, "VP9a"), ("VP9b", 1956, "V3"),
  ("V3", 2091, "V4")]

partial def walkCollectV (n : Nat) (σ : ExecState) (c : Config) (ch : Choices)
    (stopAt : Nat) (acc : List (String × ExecState × Config)) :
    IO (List (String × ExecState × Config)) := do
  let acc ← match vSched.find? (fun (i, _) => i == n) with
    | some (_, name) => do
  ...
  match stepFn σ c ch with
  | .ok (c2, σ2, ch2) => walkCollectV (n+1) σ2 c2 ch2 stopAt acc
...
#eval show IO Unit from do
  ...
  | .ok (c₀, s₃, _, ch₁) =>
  match VoteGen.walkToAnchor 0 s₃ c₀ ch₁ 200000 with
  ...
  let states ← VoteGen.walkCollectV 0 σP cA chA 19291 []
  if states.length != VoteGen.vSched.length then
    IO.println s!"COLLECT MISMATCH: {states.length} of {VoteGen.vSched.length}"; return
  ...
  let mut mstates : List (String × SymState × SymConfig) :=
    [("V0", VoteGen.reflectS σP, reflectC symDom cA)]
  let mget := fun (ms : List (String × SymState × SymConfig)) (name : String) =>
    (ms.find? (fun (n, _, _) => n == name)).getD
      ("MISSING", VoteGen.reflectS σP, reflectC symDom cA)
  let mut allOk := true
  for (a, len, b) in VoteGen.vSegs do
    ...
    if b.startsWith "VP" && b.endsWith "a" then
      let postName := String.ofList b.toList.dropLast ++ "b"
      let (_, σpost, cpost) := get postName
      let (changed, appended) ← VoteGen.heapDiff σb.heap σpost.heap
      IO.println s!"    cross {b}->{postName}: changed={changed.length} appended={appended.length} :: {VoteGen.cfgHead cb}"
      let Spost := VoteGen.applyDiff S' changed appended σpost.nextAddr
      -- post config: spill => shared .next extract; mapIter/pick => reflect
      let Cpost := match C' with
        | .retV _ (.stmtOpK _ _ _ _ _ k') => (.next k' : SymConfig)
        | _ => reflectC symDom cpost
  ...
  IO.println "vote-round mirror propagation γ-valid at all boundaries + crossings"
  -- 3. print
  let hdrCommon := "import GoLeanProofs.Sym.Mirror\n\n/-! GENERATED (A4-U23; `artifacts/probe/RoundVoteGen.lean` — DO NOT\nEDIT BY HAND). The MsgVote real-vote-family FULL-ROUND literals\n(anchor to anchor, 19,291 steps, 9 draws), tree-propagated; crossing\nposts are diff-applications over their pre-states (the U22 template,\nuniform over spills AND mapIter crossings). The window LINK theorems\nin `RoundVoteEq*.lean` re-check every literal against the mirror. -/\n\nnamespace GoLean.RaftSeam.RoundVote\n\nopen GoLean GoLean.GoCore GoLean.Sym\n\nset_option maxRecDepth 8000000\n\n"
  let names := VoteGen.vSched.map (fun (_, n) => n)
  let mut out := hdrCommon
  for nm in names do
    let (fnd, Sx, Cx) := mget mstates nm
    if fnd == "MISSING" then IO.println s!"PRINT MISSING {nm}"; return
    out := out ++ BfLitGen.emitDef s!"mvS{nm}" "SymState" (BfLitGen.pState Sx)
    out := out ++ BfLitGen.emitDef s!"mvC{nm}" "SymConfig" (BfLitGen.pC Cx)
  out := out ++ "end GoLean.RaftSeam.RoundVote\n"
  IO.FS.writeFile "../artifacts/probe/gen/RoundVoteLit.lean.gen" out
  IO.println s!"written RoundVoteLit.lean.gen: {out.length} chars"
  let (_, σ4, _) := get "V4"
  IO.println s!"V4: na={σ4.nextAddr} heap={σ4.heap.length}"
