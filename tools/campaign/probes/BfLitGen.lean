...
#eval show IO Unit from do
  ...
  let header := "import GoLeanProofs.Sym.TableExt\n\n" ++
    "/-! # A4-U4 slice 0 (GENERATED — DO NOT EDIT BY HAND)\n\n" ++
    "The becomeFollower chain's window-output states and configs as\n" ++
    "SOURCE LITERALS, generated by `artifacts/probe/BfLitGen.lean` from\n" ++
    "the probe-validated chain (BfU3Probe/BfFixture provenance). Pure\n" ++
    "data — downstream facts reduce against these literals instead of\n" ++
    "re-evaluating window chains (the U3 scale verdict's named fix for\n" ++
    "kernel-eval wall time). Correctness is NOT assumed from generation:\n" ++
    "the window LINK theorems in `BfFixture.lean` (`uW*_out`, kernel\n" ++
    "`rfl` against the evaluator) re-check every literal; on any fixture\n" ++
    "change they fail loudly — regenerate with the probe and re-check.\n" ++
    "-/\n\n" ++
    "namespace GoLean.RaftSeam\n\n" ++
    "open GoLean GoLean.GoCore GoLean.Sym\n\n" ++
    "set_option maxRecDepth 1000000\n\n"
  let mut out := header
  -- window-output states and configs (odd chain indices + final)
  let entries : List (String × SymState × SymConfig) :=
    [("1", uS1, uC1), ("3", uS3, uC3), ("5", uS5, uC5), ("7", uS7, uC7),
     ("9", uS9, uC9), ("11", uS11, uC11), ("13", uS13, uC13)]
  for (i, S, C) in entries do
    out := out ++ emitDef s!"uS{i}" "SymState" (pState S)
    out := out ++ emitDef s!"uC{i}" "SymConfig" (pC C)
    IO.println s!"printed uS{i}/uC{i}: heap {S.heap.length} cells"
  out := out ++ "end GoLean.RaftSeam\n"
  IO.FS.writeFile "../artifacts/probe/gen/BfLit.lean.gen" out
