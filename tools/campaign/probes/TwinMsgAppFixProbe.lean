...
/-- The doctored MsgApp, THE APPEND-AND-COMMIT FAMILY at the anchor's
real log (AnchorStorProbe: node 2 boots from a snapshot at (index 1,
term 1) — MemoryStorage ents = [dummy (1,1)], lastIndex 1, committed
= applied = 1, unstable.offset 2): Type 3 (na), To 2 (na+1), From 1
(na+2), Term nil (0 — the local family, the U18 doctor convention),
LogTerm → na+11 (1) and Index → na+12 (1) = the match at the real
last entry, Entries = one-entry slice (backing na+10 → Entry na+9
with Term→na+7 (1), Index→na+8 (2), Type nil, Data empty), Commit →
na+6 (2). maybeAppend appends index 2 and commitTo(2) fires — the
Ready cycle carries an unstable entry + a commit bump, driving the
storage-resp arms. Field orders: hbMsgVal / BfFixture uEntryVal. -/
      ...
      | some (nA, σA, cA, chA) =>
          ...
          | some σD =>
              ...
              | some tl =>
                ...
                IO.println s!"PRE : node2 {readLog σD tl 1}"
                ...
                | .ok (Δ, σ', c', ch') =>
                    ...
                    IO.println s!"POST: node2 {readLog σ' tl 1}"
