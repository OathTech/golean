...
open BfLitGen Bc31Gen GoLean GoLean.GoCore GoLean.GoCore.Machine GoLean.Sym GoLean.Surface GoLean.RaftSeam in
#eval show IO Unit from do
  ...
  let header := "import GoLeanProofs.Sym.TableExt\n\n" ++
    ...
    "The RE-SITED becomeCandidate chain's window-output states and\n" ++
    "configs as SOURCE LITERALS, generated by\n" ++
    "`artifacts/probe/Bc31Gen.lean` (fixture: +31 off the static range,\n" ++
    "statics 18/19 fixed). Correctness: the window LINK theorems in\n" ++
    "`Bc31.lean` (kernel rfl) re-check every literal — the drift\n" ++
  ...
  for (budget, tag) in sched do
    ...
    if n != budget then
      ...
      return
    out := out ++ emitDef s!"bc31S{idx}" "SymState" (pState S1)
    out := out ++ emitDef s!"bc31C{idx}" "SymConfig" (pC C1)
    IO.println s!"printed bc31S{idx}/bc31C{idx}: heap {S1.heap.length} cells"
    S := S1; C := C1
    idx := idx + 2
    match tag with
  ...
  IO.FS.writeFile "../artifacts/probe/gen/Bc31Lit.lean.gen" out
  IO.println s!"total generated size: {out.length} chars"
  -- MACHINE VALIDATION: end-to-end at the witness valuation/stream
  let σ0 := γS bcρw wBase bc31S0
  let c0 := γC bcρw bc31C0
  match stepFnIter 3282 σ0 c0 [3, 1, 0, 0] with
  | .ok (c, σfin, ch) =>
      let stop := match c with | Machine.Config.next .stop => true | _ => false
      let γfin := γS (uρ' bcρw 3 1 0) wBase S  -- S = mirror final (bc31S13)
      ...
      IO.println s!"spec = {toString (repr (specBecomeCandidate ⟨0, 7, 2, 0, 1, 1⟩ 1))}"
