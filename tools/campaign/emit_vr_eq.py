#!/usr/bin/env python3
"""A4-U25: emit RoundVrEq* modules from the auto-discovery manifest.
Reads the MANIFEST section of roundvrgen.out; groups segments into
...
for gi, g in enumerate(groups):
    ...
    for it in g:
        if it[0]=='SEG':
            winN += 1
            nm = f"vrW{L}{winN}_out"
            body.append(f"theorem {nm} : symEvalWindowTB bfTB {it[2]} vrS{it[1]} vrC{it[1]}\n    = ({it[2]}, vrS{it[3]}, vrC{it[3]}) := by\n  kernel_rfl\n")
            ...
            if consumed == 1:
                thm = f"roundVr_x{crossN}"
                if kind == 'reflect' and crossN > 1:
                    # Visit-mapIter class: table-consulting at free σ (the U23
                    # finding) — carry the windows' Agrees premise and pin
                    # through γS_pin (SymTables.Agrees.concS_eq, promoted U24).
                    body.append(f"/-- Crossing {crossN} ({kind}; latitude: map-iteration order; table-pinned per the U23 finding). -/\ntheorem {thm} (ρ : Valuation) (σ : ExecState)\n    (hag : bfTB.Agrees σ) (rest : Choices) :\n    stepFn (γS ρ σ vrS{cpre}) (γC ρ vrC{cpre}) (0 :: rest)\n      = .ok (γC ρ vrC{cpost}, γS ρ σ vrS{cpost}, rest) := by\n  simp only [RoundVote.γS_pin hag]\n  kernel_rfl\n")
                    remaining -= 1
                    haves.append(f"  have h{len(haves)} := {thm} ρ σ hag ({stream(remaining)})")
                else:
    ...
    span = f"/-- Segment span {L}: {gsteps} steps, {gdraws} draw(s). -/\ntheorem roundVr_span{L} (ρ : Valuation) (σ : ExecState)\n    (hag : bfTB.Agrees σ) (rest : Choices) :\n    stepFnIter {gsteps} (γS ρ σ vrS{pre}) (γC ρ vrC{pre}) ({stream(gdraws)})\n      = .ok (γC ρ vrC{post}, γS ρ σ vrS{post}, rest) := by\n" + '\n'.join(haves) + f"\n  exact {expr}\n"
    mods.append((L, lits, body, span, gsteps, gdraws, pre, post))

for L, lits, body, span, gsteps, gdraws, pre, post in mods:
    imports = '\n'.join(f"import GoLeanProofs.Specs.Raft.RoundVrLit{n}" for n in lits)
    hdr = f"""{imports}
...
/-! # RoundVrEq{L} — segment {L} of the MsgVoteResp election-completion
round's canonical run (A4-U25): {pre} -> {post}, {gsteps} steps,
...
"""
    with open(f"RoundVrEq{L}.lean", 'w') as f:
        f.write(hdr + '\n'.join(body) + '\n' + span + '\nend GoLean.RaftSeam.RoundVr\n')
    print(f"RoundVrEq{L}.lean: {gsteps} steps {gdraws} draws lits={lits} pre={pre} post={post}")
