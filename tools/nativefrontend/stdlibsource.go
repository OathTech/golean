...
var stdlibSourceAllowed = map[string]string{
	...
	"cmp":                  "slice-2 target; Compare is the REAL generic at EVERY type argument since 2026-09-04 (lane fr24: the retained integer/string kind-dispatch desugar RETIRED per the [USER] ruling «(2) … an honest red», relayed — a function-local defined type argument now refuses at mono.go's C6 naming rule, rows slices/sortfunc-cmp/cmp-compare-kinds + stdlib-source/cmp-compare/local-float-type on FR-19's line; NaN arm included); pure, no imports; init-pure: no package-level state",
