...
file open with no wrapper in the loop. CI wraps each gate step in a
SYSTEM-scope cgroup cap via passwordless sudo (hosted runners have no user
bus for `scripts/capped` itself), and the cap is PROVEN, not asserted: the
workflow passes `GOLEAN_CAPPED=1` + the real size, so `scripts/ci` re-execs
through `capped`'s readback, which refuses if the kernel disagrees
(2026-08-14/15): a runner VM that dies uploads no logs and saves no cache,
so "disposable, let it die" was the wrong frame —
`docs/2026-08-14_ci-runner-death-and-slow-tier-timeout.md`.
