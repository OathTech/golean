...
G1.9, COMPLETED by WP arc slice 4 phase 3 — the chartered acceptance),
in the shape the phase-2 shards use. The shard imports ONLY this
example's root — which reaches the HEADLINE, because the headline is
stated in the root rather than in a leaf shard (the C-H4/C-H5 shape,
adopted from birth).

PROVENANCE NOTE, recorded because it matters here more than anywhere:
a first version of this shard was WITHDRAWN UNVERIFIED with the
campaign (its module had never elaborated — g1.md unit G1.9). Every
pin below is checked IN THIS BUILD by `#guard_msgs`; nothing is
inherited from the withdrawn shard's text.
...
**PROOF ROUTE (the GAP-RFL-COST closure, in two parts):** the three
291-step `seedMat(seed)` outer-iteration segments — the windows the
campaign MEASURED as the blockers (61–326 s raw; 61.4 s re-measured at
this tree) — are discharged through the mirror symbolic evaluator
(`symEvalWindow_refines'`, `GoLean.Sym`) at ~1.3 s each — a PROOF
DEVICE only: no `GoLean.Sym` constant appears in any statement here,
and the axiom pins below cover the transported route (the refinement
theorem is classical-trio). Every other window stays raw `rfl` under
`set_option smartUnfolding false`: landing this file found the class's
ROOT CAUSE — a MetaM smart-unfolding pathology, elaboration-side only
(the kernel re-checked the same proofs in milliseconds throughout);
under the option the worst raw segment takes 1.09 s. Statements are
byte-identical to the probe-confirmed forms; the two-stage numbers and
the root-cause record are `docs/wp-arc-log/s4.md` unit S4.11.
...
`#eval` (at `2^64−1`, `2^64−2`, `2^64−6`, re-run at THIS tree), not
oracle-pinned.
...
iterations). Bound and probe measurement COINCIDE (both 5247,
re-probed at THIS tree at `seed = 0`, `5`, `2^63−1`); the proof's 82
chained segment counts sum to it. Neither is presented as the other.
**`∀ ch` is vacuous and stated anyway** — the run consumes no choice.

Deletion-test record (RUN by this landing, not inherited): `seed` and
`hseed` are the only explicit binders; `hseed` is PROOF-STRUCTURE
load-bearing (it discharges the seed's uint64 normal form at the entry
equation and feeds every `omega` bridge), NOT truth-load-bearing at the
margin — a machine probe at `seed = 2^64` still matches the
`aClean`-family postcondition (entry normalization and the family
reduce mod 2^64 together, the `queue` entry's `hseed` classification
exactly). The probe record is in the gallery entry.
...
no `mmAcc`, no `unn`, no family function, and NO `GoLean.Sym` constant.
...
/-! ## The matmul example (Gallery Campaign G1 unit G1.9; landed by WP
arc slice 4 phase 3) -/
...
/-- info: 'GoLean.Examples.MatMul.un_idem' depends on axioms: [propext, Quot.sound] -/
