...
/-!
# W1 pilot scaffolding: the hand crossing facts (HARVESTED from
...
/-- The raw pop, in closed form over ANY stream (W1: the judgment's
∀ ch includes short streams — exhaustion yields 0 without popping,
and `headD 0`/`tail` express both cases uniformly). -/
theorem consume_eq (ch : Choices) (b : Nat) :
    Choices.consume ch b = (ch.headD 0 % max 1 b, ch.tail) := by
  cases ch with
  | nil => simp [Choices.consume]
  | cons c rest => simp [Choices.consume]
...
def uB1 : Stmt := match uC1 [] .stop with
  | .next (.mapIterK _ _ _ _ b _ _ _ _ _) => b
  | _ => .seqn #[]
def uE1 : LocalEnv := match uC1 [] .stop with
  | .next (.mapIterK _ _ _ _ _ _ _ _ e _) => e
  | _ => []
def uK1 (tenv : LocalEnv) (k : SymCont) : GoLean.Sym.Cont symDom :=
  match uC1 tenv k with
  | .next (.mapIterK _ _ _ _ _ _ _ _ _ kk) => kk
  | _ => .stop

def uStart1 : Array SymValue := match uC1 [] .stop with
...
theorem uC1_shape (tenv : LocalEnv) (k : SymCont) :
    uC1 tenv k = .next (.mapIterK (some "k") none uTyU uTySt
      uB1 (some (.base ⟨32⟩)) #[] uStart1 uE1 (uK1 tenv k)) := by
  kernel_rfl
...
theorem uC2_eq (tenv : LocalEnv) (k : SymCont) : uC2 tenv k = .exec uB1
    ((uE1.pushScope).declare "k" (uS1.alloc uKeyV1 (some uTyU)).1)
    (.mapIterK (some "k") none uTyU uTySt uB1 (some (.base ⟨32⟩))
      (Array.push #[] uKeyV1) uStart1 uE1 (uK1 tenv k)) := by
  unfold uC2 uP2
  rw [uC1_shape tenv k]
  rfl
...
theorem uPick1_step (tenv : LocalEnv) (k : SymCont) (ρ : Valuation)
    (σ : ExecState)
    (hag : bfTB.Agrees σ) (b c d : Int) (ch : Choices) :
    stepFn (γS (uρ ρ ((ch.headD 0 % 10 : Nat) : Int) b c d) σ uS1)
      (γC (uρ ρ ((ch.headD 0 % 10 : Nat) : Int) b c d) (uC1 tenv k)) ch
      = .ok (γC (uρ ρ ((ch.headD 0 % 10 : Nat) : Int) b c d) (uC2 tenv k),
          γS (uρ ρ ((ch.headD 0 % 10 : Nat) : Int) b c d) σ uS2,
          ch.tail) := by
  have h10 : ch.headD 0 % 10 < 10 := Nat.mod_lt _ (by decide)
  rw [uC1_shape tenv k, uC2_eq tenv k, uS2_eq]
  ...
  · show Choices.consume ch (uCands1.size + _) = _
    rw [consume_eq]
    rfl
...
def uB3 : Stmt := match uC3 [] .stop with
  | .next (.mapIterK _ _ _ _ b _ _ _ _ _) => b
  | _ => .seqn #[]
def uE3 : LocalEnv := match uC3 [] .stop with
  | .next (.mapIterK _ _ _ _ _ _ _ _ e _) => e
  | _ => []
def uK3 (tenv : LocalEnv) (k : SymCont) : GoLean.Sym.Cont symDom :=
  match uC3 tenv k with
  | .next (.mapIterK _ _ _ _ _ _ _ _ _ kk) => kk
  | _ => .stop
def uStart3 : Array SymValue := match uC3 [] .stop with
  | .next (.mapIterK _ _ _ _ _ _ _ st _ _) => st
  | _ => #[]
...
theorem uC3_shape (tenv : LocalEnv) (k : SymCont) :
    uC3 tenv k = .next (.mapIterK (some "id") none uTyK uTyProg
      uB3 (some (.base ⟨2⟩)) #[] uStart3 uE3 (uK3 tenv k)) := by
  kernel_rfl
...
theorem uPick2_step (tenv : LocalEnv) (k : SymCont) (ρ : Valuation)
    (σ : ExecState)
    (a c d : Int) (ch : Choices) :
    stepFn (γS (uρ ρ a (uKey1 (ch.headD 0)) c d) σ uS3)
      (γC (uρ ρ a (uKey1 (ch.headD 0)) c d) (uC3 tenv k)) ch
      = .ok (γC (uρ ρ a (uKey1 (ch.headD 0)) c d) (uC4 tenv k),
          γS (uρ ρ a (uKey1 (ch.headD 0)) c d) σ uS4, ch.tail) := by
  have h3 : ch.headD 0 % 3 < 3 := Nat.mod_lt _ (by decide)
  rw [uC3_shape tenv k, uC4_eq tenv k, uS4_eq]
  refine stepFn_pick_transport _ σ (uCands3_fact _ σ)
    (by with_unfolding_all rfl) (by with_unfolding_all rfl)
    ?_ (uCands3_get _ h3) (by with_unfolding_all rfl) ?_
  · show Choices.consume ch (uCands3.size + _) = _
    rw [consume_eq]
    rfl
