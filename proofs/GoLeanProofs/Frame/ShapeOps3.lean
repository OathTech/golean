...
/-!
# S-copies, part 3: the channel/sync/select arms (`ChanSync` mirrors)

Mechanical copies of the mutating chan/sync/select apply lemmas with
the strengthened relation (`CfgSimS`, `FrameSimS`) through
`storeLoc_simS`; the read-only cell/readiness lemmas are consumed
weak via `toFrameSim`. See `ShapeSim.lean` for the scaffold record.
-/
...
/-- `CfgSim` at the strengthened relation. -/
def CfgSimS (ρ : Nat → Nat) (na₀ na : Nat) (fr : Heap) :
    Config × ExecState → Config × ExecState → Prop :=
  fun r rF => rF.1 = renameConfig ρ r.1 ∧ FrameSimS ρ na₀ na fr r.2 rF.2

section
variable {ρ : Nat → Nat} {na₀ na : Nat} {fr : Heap} {σ σF : ExecState}

theorem renamedArray_push (buf : Array GoValue) (v : GoValue) :
...
theorem renamedArray_eraseIdx!_zero {buf : Array GoValue}
    (h : 0 < buf.size) :
...
theorem renameAssigneeList_length (l : List Assignee) :
    (renameAssigneeList ρ l).length = l.length := by
  simp [renameAssigneeList]

theorem renameStmt_seqn_empty :
    renameStmt ρ (.seqn #[]) = .seqn #[] := by
  simp [renameStmt, renameStmtList]

theorem renameConfig_panicking_rt (msg : String) (k : Cont) :
...
theorem renameConfig_panicking_str (msg : String) (k : Cont) :
...
theorem valueAsIntCS_sim (v : GoValue) :
...
theorem listSim_imp {α β : Type} {R S : α → β → Prop}
    (h : ∀ a b, R a b → S a b) :
...
theorem mapM_sim {α β γ δ : Type} {t : α → β} {R : γ → δ → Prop}
...
theorem enterRecvTargets_simS (hS : FrameSimS ρ na₀ na fr σ σF)
    (targets : List Assignee) (vals : List GoValue) (body : Stmt)
    (env : LocalEnv) (k : Cont) :
    ExSim (CfgSimS ρ na₀ na fr)
...
/-- `enterRecvTargets_simS` with the framed value list and body given by
...
theorem applyChanOp_simS (hS : FrameSimS ρ na₀ na fr σ σF)
      ...
      | cons chv t =>
          ...
          | cons vv t2 =>
              ...
              | nil =>
                  ...
                  | some loc =>
                      refine ExSim.bind (chanCell_sim hS.toFrameSim loc) ?_
                      intro c cF hc
                      ...
                      · simp only [if_neg hcl]
                        ...
                        · simp only [if_pos hsz]
                          ...
                          refine ExSim.bind (storeLoc_simS hS loc _) ?_
                          intro σ' σF' hS'
          ...
          | nil =>
              ...
              | some loc =>
                  ...
                  | some v =>
                      ...
                      | cons a ts =>
                          exact opDoneWrap_simS (enterRecvTargets_simS' hS₁ (a :: ts)
...
theorem applySyncOp_simS (hS : FrameSimS ρ na₀ na fr σ σF)
  ...
  | lock =>
      ...
      | cons av t =>
          ...
          | nil =>
              ...
              intro loc locF hloc
              subst hloc
              refine ExSim.bind (syncCell_sim hS.toFrameSim loc) ?_
              intro p pF hp
              subst hp
              cases p with
              | mutex locked =>
      ...
      | cons av t =>
          ...
          | cons dv t2 =>
              ...
              | nil =>
                  ...
                  intro loc locF hloc
                  subst hloc
                  refine ExSim.bind (valueAsIntCS_sim dv) ?_
                  intro d dF hd
                  subst hd
                  refine ExSim.bind (syncCell_sim hS.toFrameSim loc) ?_
                  intro p pF hp
          ...
          | nil =>
              ...
              | once started dn =>
                  ...
                  · simp only [if_pos hst]
                    refine ExSim.bind (storeLoc_simS hS loc _) ?_
                    intro σ' σF' hS'
                    exact opDoneWrap_simS (enterRecvTargets_simS' hS' targets [.bool true]
                      (.seqn #[]) env k (by simp [renameValueList, renameValue])
                      renameStmt_seqn_empty.symm)
                  · simp only [if_neg hst]
                    by_cases hdn : dn = true
                    · simp only [if_pos hdn]
                      exact opDoneWrap_simS (enterRecvTargets_simS' hS targets [.bool false]
                        (.seqn #[]) env k (by simp [renameValueList, renameValue])
                        renameStmt_seqn_empty.symm)
...
theorem commitClause_simS (hS : FrameSimS ρ na₀ na fr σ σF)
...
-- The SelectOutcome relation: done-config renamed + FrameSimS (with
...
-- CfgSimS; inr: same panic message).

/-- The pointwise relation of `SelectOutSimS`'s `.picks` arm, named so
the `mapM` induction and the `applySelect` pick-transfer can traverse
it without restating the match. -/
def PickRelS (ρ : Nat → Nat) (na₀ na : Nat) (fr : Heap) :
    (EvClause × Sum (Config × ExecState) String) →
      (EvClause × Sum (Config × ExecState) String) → Prop
  | (cl, x), (clF, xF) =>
      clF = renameEvClause ρ cl ∧
      match x, xF with
      | .inl r, .inl rF => CfgSimS ρ na₀ na fr r rF
      | .inr m, .inr mF => mF = m
      | _, _ => False

def SelectOutSimS (ρ : Nat → Nat) (na₀ na : Nat) (fr : Heap) :
    SelectOutcome → SelectOutcome → Prop
  | .done c s cl?, .done cF sF clF? =>
      cF = renameConfig ρ c ∧ FrameSimS ρ na₀ na fr s sF
        ∧ clF? = cl?.map (renameEvClause ρ)
  | .picks l, .picks lF => ListSim (PickRelS ρ na₀ na fr) l lF
  | _, _ => False

theorem selectOutSim_of_picksS
    {l lF : List (EvClause × Sum (Config × ExecState) String)}
    (h : ListSim (PickRelS ρ na₀ na fr) l lF) :
    SelectOutSimS ρ na₀ na fr (.picks l) (.picks lF) := h

theorem selectOutSim_picksS
    {l lF : List (EvClause × Sum (Config × ExecState) String)}
    (h : SelectOutSimS ρ na₀ na fr (.picks l) (.picks lF)) :
    ListSim (PickRelS ρ na₀ na fr) l lF := h
...
theorem applySelectCore_simS (hS : FrameSimS ρ na₀ na fr σ σF)
  ...
  refine ExSim.bind (exSim_of_ren (evalClauses_noPanicS clauses vs)
    (fun evs h => evalClauses_ren ρ h)) ?_
  intro evs evsF hevs
  subst hevs
  refine ExSim.bind (readyClauses_sim hS.toFrameSim evs) ?_
  intro r rF hr
  subst hr
  cases r with
  | nil =>
  ...
  | cons c₁ rest =>
      cases rest with
      | nil =>
          refine ExSim.bind (commitClause_simS hS env k c₁) ?_
          intro p pF hp
          have hp' : pF.1 = renameConfig ρ p.1
              ∧ FrameSimS ρ na₀ na fr p.2 pF.2 := hp
          obtain ⟨c', s'⟩ := p
          obtain ⟨cF', sF'⟩ := pF
          exact ExSim.ok ⟨hp'.1, hp'.2, rfl⟩
      | cons c₂ rest₂ =>
          refine ExSim.bind
            (mapM_sim (t := renameEvClause ρ) (R := PickRelS ρ na₀ na fr)
              ?_ (c₁ :: c₂ :: rest₂)) ?_
          · intro cl
            have hcc := commitClause_simS hS env k cl
          ...
          · intro l lF hl
            exact ExSim.ok (selectOutSim_of_picksS hl)

theorem applySelect_simS (hS : FrameSimS ρ na₀ na fr σ σF)
    (clauses : List (SelectClauseHead × Stmt)) (default? : Option Stmt)
    (vs : List GoValue) (env : LocalEnv) (k : Cont) (ch : Choices) :
    ExSim (fun (r rF : Config × ExecState × Choices × Option EvClause) =>
        rF.1 = renameConfig ρ r.1 ∧ FrameSimS ρ na₀ na fr r.2.1 rF.2.1
          ∧ rF.2.2.1 = r.2.2.1
  ...
  refine ExSim.bind (applySelectCore_simS hS clauses default? vs env k) ?_
  intro o oF ho
  cases o with
  | done c' s' cl? =>
      cases oF with
  ...
  | picks commits =>
      ...
      | picks commitsF =>
          have hls := selectOutSim_picksS ho
            ...
            | inl r =>
                ...
                | inl rF =>
                    have hr : rF.1 = renameConfig ρ r.1
                        ∧ FrameSimS ρ na₀ na fr r.2 rF.2 := hab
                    obtain ⟨c', s'⟩ := r
                    obtain ⟨cF', sF'⟩ := rF
