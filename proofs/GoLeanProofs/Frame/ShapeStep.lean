...
/-!
# The strengthened per-step simulation (`stepFn_simS`) — C2a part 4

The StepSim induction COPY-THREADED at `FrameSimS`/`TripSimS`: the
script is the weak induction's verbatim, with the mutating callee
lemmas at their S-versions and the read-only callees consumed weak
through `toFrameSim`. `applyStrictOp` is consumed from
`ShapeStrict` (the mechanical whole-file mirror): the derivation route
(state passthrough — `applyStrictOp` allocates in exactly two arms,
the C2a measurement) was measured out at two heartbeat timeouts, and
the copy is the budget-honest route.
See `ShapeSim.lean` for the scaffold record.
-/
...
/-- `bind_eq_ok` at the `Bind.bind`-spelling the zeta-unfolded
join-point bodies wear (the `>>=` spelling elaborates to `HBind` and
misses them syntactically). -/
theorem bbind_eq_ok {α β : Type} {x : Except GoError α}
    {f : α → Except GoError β} {b : β} :
    Bind.bind x f = Except.ok b ↔ ∃ a, x = Except.ok a ∧ f a = Except.ok b := by
  cases x <;> simp [Bind.bind, Except.bind]

/-- Generic inversion for the do-desugared `Except` match-bind (the
shape `match x with | .error err => .error err | .ok v => f v`). -/
theorem except_match_ok {α β : Type} {x : Except GoError α}
    {f : α → Except GoError β} {r : β}
    (h : (match x with
      | Except.error err => Except.error err
      | Except.ok v => f v) = Except.ok r) :
    ∃ a, x = .ok a ∧ f a = .ok r := by
  cases x with
  | error e => cases h
  | ok a => exact ⟨a, rfl, h⟩

/-- `applySlice` threads the state through unchanged (pure slicing). -/
theorem applySlice_state {s : ExecState} {b : GoValue} {lo hi : Int}
    {m : Option Int} {r : GoValue × ExecState}
    (h : applySlice s b lo hi m = .ok r) : r.2 = s := by
  suffices H : ∀ r2 : GoValue × ExecState,
      applySlice s b lo hi m = .ok r2 → r2.2 = s from H r h
  intro r2
  fun_cases applySlice s b lo hi m <;> intro hr <;>
    first
      | (simp only [bind_eq_ok, pure_eq_ok, Except.ok.injEq] at hr
         first
           ...
           | (rw [hr]; done)
           | (obtain ⟨d2, -, hr2⟩ := hr
              first | (rw [← hr2]; done) | (rw [hr2]; done)))
      | (simp at hr; done)
      | (cases hr; done)
      | (obtain ⟨d2, -, hr⟩ := except_match_ok hr
         dsimp only at hr
         first
           | (simp only [bind_eq_ok, pure_eq_ok, Except.ok.injEq] at hr
              first
...
theorem renameStmt_seqn_emptySS (ρ : Nat → Nat) :
    renameStmt ρ (.seqn #[]) = .seqn #[] := by
  simp [renameStmt, renameStmtList]

theorem renameEnv_nil (ρ : Nat → Nat) : renameEnv ρ ([] : LocalEnv) = [] := rfl
theorem renameChain_nil (ρ : Nat → Nat) : renameChain ρ [] = [] := rfl
theorem renameChain_cons (ρ : Nat → Nat) (e : PanicEntry)
    (es : List PanicEntry) :
    renameChain ρ (e :: es) = renameEntry ρ e :: renameChain ρ es := rfl
theorem renameDefers_nil (ρ : Nat → Nat) : renameDefers ρ [] = [] := rfl
theorem renameDefers_cons (ρ : Nat → Nat) (d : GoValue × List GoValue)
    (ds : List (GoValue × List GoValue)) :
    renameDefers ρ (d :: ds) = renameDefer ρ d :: renameDefers ρ ds := rfl
theorem renamePlans_nil (ρ : Nat → Nat) : renamePlans ρ [] = [] := rfl
theorem renamePlans_cons (ρ : Nat → Nat) (p : TargetShape × List Expr)
    (ps : List (TargetShape × List Expr)) :
    renamePlans ρ (p :: ps) = (p.1, renameExprList ρ p.2) :: renamePlans ρ ps := rfl
theorem renameAssigneeList_nil (ρ : Nat → Nat) :
    renameAssigneeList ρ [] = [] := rfl
theorem renameAssigneeList_cons (ρ : Nat → Nat) (a : Assignee)
    (as' : List Assignee) :
    renameAssigneeList ρ (a :: as') = renameAssignee ρ a :: renameAssigneeList ρ as' := rfl

theorem list_eraseIdx_mapSS {α β : Type} (f : α → β) :
...
theorem targetsPlan_pair_ren {ρ : Nat → Nat} (a b : Assignee) :
...
rewrite). -/
def wideEntry (σ₀ : ExecState) (op : StmtOp) (nt : Nat)
...
theorem stepFn_exec_wide {σ₀ : ExecState} {s : Stmt} {env : LocalEnv}
...
def strictEntry (σ₀ : ExecState) (op : StrictOp) (es : List Expr)
...
theorem stepFn_evalE_strict {σ₀ : ExecState} {e : Expr}
...
theorem stepFn_exec_chanRecv {σ₀ : ExecState}
...
theorem stepFn_exec_sync {σ₀ : ExecState} {sop : SyncStmtOp}
...
theorem stepFn_exec_assignMany {σ₀ : ExecState}
...
theorem completeTargetRef_ren_cons (ρ : Nat → Nat) (sh : TargetShape)
...
macro "ren_simpS_only" hinj:term : tactic =>
  `(tactic| simp only [stepFn, renameConfig, renameCont, renameStmt,
      ...
      renameStmt_seqn_emptySS, targetsPlan_map_ren,
...
macro "ren_simpS_all" hinj:term : tactic =>
...
theorem stepFn_simS {ρ : Nat → Nat} {na₀ na : Nat} {fr : Heap}
    {σ σF : ExecState}
    (hS : FrameSimS ρ na₀ na fr σ σF) (c : Config) (ch : Choices) :
  ...
  case case7 =>
    ...
    have hren := renderPanicHead_ren (ρ := ρ) hS.toFrameSim hinj first rest
    rw [hrend] at hren
  ...
  case case13 =>
    ...
    refine ExSim.bind (allocDecls_simS _ _ hS) ?_
    intro r rF hr
    obtain ⟨env1, σ1⟩ := r
    obtain ⟨env1F, σ1F⟩ := rF
    obtain ⟨henv1, hS1⟩ := hr
  ...
  case case36 =>
    ren_simpS_all hinj
    refine enterFrameStep_simS hS _ _ _ _ _ _ ?_
    intro f e ls hbody
    ren_simpS_only hinj
    rw [hbody]
  ...
  case case66 =>
    ...
    ren_simpS_only hinj
    rw [hlook]
    refine ExSim.bind (loadLoc_sim hS.toFrameSim _) ?_
    intro v vF hvF
    subst hvF
    exact ExSim.ok ⟨rfl, hS, rfl⟩
  ...
  case case3 =>
    ...
    refine enterFrameDeferPanicking_simS hS _ _ _ _ _ _ _ ?_
    intro f e hbody
    ren_simpS_only hinj
    rw [hbody]
  ...
  case case117 =>
    ren_simpS_only hinj
    refine ExSim.bind (mapRangeStartSets_sim hS.toFrameSim _) ?_
    intro bs bsF h
    obtain ⟨b1, s1⟩ := bs
    obtain ⟨b1F, s1F⟩ := bsF
    obtain ⟨h1, h2⟩ := h
  ...
  case case120 =>
    ...
    have hsim := applyChanOp_simS (ρ := ρ) hS op ((v :: done).reverse) env k'
  ...
  case case121 =>
    ...
    ren_simpS_only hinj
    rw [hF]
    exact ExSim.ok ⟨by ren_simpS_only hinj, hS, rfl⟩
  -- select apply: ok / panic
  case case124 =>
    rename_i v clauses default? done env k' c' s' choices' cl? happ
    have hsim := applySelect_simS (ρ := ρ) hS clauses default?
  ...
  case case143 =>
    rename_i v op done env k' c' s' happ
    have hsim := applySyncOp_simS (ρ := ρ) hS op ((v :: done).reverse) env k'
    simp only [renameValueList_reverse, renameValueList] at hsim
    obtain ⟨rF, hF, hrel⟩ := hsim.ok_inv happ
    obtain ⟨cF, sF2⟩ := rF
    obtain ⟨hc, hst⟩ := hrel
  ...
  case case153 =>
    ren_simpS_only hinj
    refine ExSim.bind (loadMany_sim hS.toFrameSim _) ?_
    intro vs vsF h
    exact ExSim.stuck'
  case case192 =>
    ren_simpS_only hinj
  ...
  case case163 =>
    ...
    ren_simpS_only hinj
    refine ExSim.bind (mapIterCandidates_sim hinj hS.toFrameSim keyTy valTy base produced) ?_
    ...
    · rw [if_neg hemp, if_neg hemp]
      ...
      | some kv =>
          ...
          rw [← localEnv_pushScope_ren]
          refine ExSim.bind (bindIterVars_simS hS _ _ _ _ _ _ _) ?_
          intro r rF hr
          obtain ⟨env1, σ1⟩ := r
          obtain ⟨env1F, σ1F⟩ := rF
          obtain ⟨henv1, hS1⟩ := hr
  ...
  case case64 =>
    ...
    rw [stepFn_exec_wide hplanF]
    simp only [wideEntry]
    refine ExSim.bind (applyStmtOp_simS hS hinj ch op nt []) ?_
    intro r rF hrel
    obtain ⟨s1, c1⟩ := r
    obtain ⟨s2, c2⟩ := rF
    obtain ⟨hst, hc⟩ := hrel
  ...
  case case79 =>
    ...
    rw [stepFn_evalE_strict hplanF]
    simp only [strictEntry]
    have hsim := applyStrictOp_sim_SS (ρ := ρ) hS hinj op []
  ...
  case case84 =>
    ...
    have hsim := applyStrictOp_sim_SS (ρ := ρ) hS hinj op ((v :: done).reverse)
    simp only [renameValueList_reverse, renameValueList] at hsim
    obtain ⟨rF, hF, hrel⟩ := hsim.ok_inv happ
    obtain ⟨vF, sF2⟩ := rF
    obtain ⟨hv, hst⟩ := hrel
  ...
  case case98 =>
    ...
    have hsim := applyStmtOp_simS (ρ := ρ) hS hinj ch op nt ((v :: done).reverse)
    simp only [renameValueList_reverse, renameValueList] at hsim
    obtain ⟨rF, hF, hrel⟩ := hsim.ok_inv happ
    obtain ⟨sF2, chF2⟩ := rF
    obtain ⟨hst, hch⟩ := hrel
    ...
    have hcsim := contAfterStmtOp_sim hinj hst.toFrameSim op ((v :: done).reverse) k'
  ...
  case case134 =>
    ...
    have hsim := applyRhsOp_sim (ρ := ρ) hS.toFrameSim hinj rop ((v :: done).reverse)
    simp only [renameValueList_reverse, renameValueList] at hsim
  ...
  case case164 =>
    ...
    have hsim := storeTarget_simS (ρ := ρ) hS hinj r val
