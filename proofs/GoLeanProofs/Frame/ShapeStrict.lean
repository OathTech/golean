import GoLeanProofs.Frame.ShapeOps

/-!
# S-copy of the strict-op arm layer (`StrictOps` mirror, C2a)

MECHANICAL transform of `Frame/StrictOps.lean` at the strengthened
relation: every declaration renamed with an `_SS` suffix, `FrameSim`
→ `FrameSimS` (parent-field access and the S-primitives carry the
same dot-names), external weak read-only lemmas consumed via
`toFrameSim`. Generated by the C2a build script recorded in the U19
log; the DERIVATION route (state passthrough for the non-allocating
arms) was measured out at two heartbeat timeouts — the copy is the
budget-honest route. Scaffold record: `ShapeSim.lean`.
-/

namespace GoLean.Frame


open GoLean GoLean.GoCore GoLean.GoCore.Machine

set_option linter.unusedSimpArgs false

section
variable {ρ : Nat → Nat} {na₀ na : Nat} {fr : Heap} {σ σF : ExecState}

/-! ## Generic `ExSim` utilities -/

/-- `ExSim.bind` with the canonical `.ok` hypothesis available in the
continuation (the typeAssert arm needs its ok-shape inversion). -/
theorem exSim_bind_ok_SS {α β γ δ : Type} {R : α → β → Prop}
    {S : γ → δ → Prop} {x : Except GoError α} {y : Except GoError β}
    {f : α → Except GoError γ} {g : β → Except GoError δ}
    (hxy : ExSim R x y)
    (hfg : ∀ a b, x = .ok a → R a b → ExSim S (f a) (g b)) :
    ExSim S (x >>= f) (y >>= g) := by
  cases x with
  | ok a =>
      cases y with
      | ok b => simpa [Bind.bind, Except.bind] using hfg a b rfl hxy
      | error e => exact absurd hxy (by simp [ExSim])
  | error e =>
      cases e with
      | panic m =>
          have hy : y = .error (.panic m) := hxy
          subst hy
          exact rfl
      | _ => trivial

/-- Same computation on both sides, ok-results fixed by the renaming. -/
theorem exsim_ren_self_SS (ρ : Nat → Nat) {x : Except GoError GoValue}
    (hok : ∀ v, x = .ok v → renameValue ρ v = v) :
    ExSim (fun a b => b = renameValue ρ a) x x := by
  cases x with
  | ok a => exact (hok a rfl).symm
  | error e => cases e <;> first | rfl | trivial

/-- Same-value transfer plus panic-freedom, at relation `Eq`. -/
theorem exsim_eq_of_ren_SS {α : Type} {x y : Except GoError α}
    (hnp : NoPanic x) (htr : ∀ a, x = .ok a → y = .ok a) : ExSim Eq x y := by
  cases x with
  | ok a => rw [htr a rfl]; exact ExSim.ok rfl
  | error e =>
      cases e with
      | panic m => exact absurd rfl (hnp m)
      | _ => trivial

/-- The do-guard shape (`if c then stuck/panic …` with the continuation
duplicated into both branches by the elaborator), same condition on
both runs. -/
theorem exsim_stepSim_refl_SS {β : Type}
    (x : Except GoError (ForInStep β)) : ExSim (StepSim Eq) x x := by
  refine (ExSim.refl x).weaken ?_
  intro a b h
  subst h
  cases a <;> exact rfl

/-- Package a value-level transfer into the `applyStrictOp` result pair
(state unchanged on both sides). -/
theorem scalar_pair_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    {x y : Except GoError GoValue}
    (h : ExSim (fun v vF => vF = renameValue ρ v) x y) :
    ExSim (fun (r rF : GoValue × ExecState) =>
        rF.1 = renameValue ρ r.1 ∧ FrameSimS ρ na₀ na fr r.2 rF.2)
      (x >>= fun v => pure (v, σ)) (y >>= fun v => pure (v, σF)) :=
  ExSim.bind h fun _ _ hb => ExSim.ok ⟨hb, hS⟩

/-! ## Renamed-array/entry bridges -/

theorem renArr_size_SS (vs : Array GoValue) :
    ((renameValueList ρ vs.toList).toArray).size = vs.size := by
  simp [renameValueList_eq_map]

theorem renArr_push_SS (vs : Array GoValue) (v : GoValue) :
    ((renameValueList ρ vs.toList).toArray).push (renameValue ρ v)
      = (renameValueList ρ (vs.push v).toList).toArray := by
  simp [renameValueList_eq_map]

theorem renEntries_size_SS (es : Array (GoValue × GoValue)) :
    ((renameValueEntries ρ es.toList).toArray).size = es.size := by
  simp [renameValueEntries_eq_map]

theorem renEntries_getElem? (es : Array (GoValue × GoValue)) (i : Nat) :
    ((renameValueEntries ρ es.toList).toArray)[i]?
      = (es[i]?).map (fun p => (renameValue ρ p.1, renameValue ρ p.2)) := by
  simp [renameValueEntries_eq_map, ← Array.getElem?_toList]

/-! ## `valueAs*` coercion simulations -/

theorem valueAsInt_sim_SS (ρ : Nat → Nat) {v vF : GoValue}
    (h : vF = renameValue ρ v) :
    ExSim Eq (valueAsInt v) (valueAsInt vF) := by
  subst h
  cases v <;> simp only [renameValue] <;>
    first
    | exact ExSim.stuck'
    | exact ExSim.ok rfl

theorem valueAsBool_sim_SS (ρ : Nat → Nat) (v : GoValue) :
    ExSim Eq (valueAsBool v) (valueAsBool (renameValue ρ v)) := by
  cases v <;> simp only [renameValue] <;>
    first
    | exact ExSim.stuck'
    | exact ExSim.ok rfl

theorem valueAsSlice_sim_SS (ρ : Nat → Nat) (v : GoValue) :
    ExSim (fun sl slF => slF = { sl with base := sl.base.map (renameLoc ρ) })
      (valueAsSlice v) (valueAsSlice (renameValue ρ v)) := by
  cases v <;> simp only [renameValue] <;>
    first
    | exact ExSim.stuck'
    | exact ExSim.ok rfl

theorem valueAsMap_sim_SS (ρ : Nat → Nat) (v : GoValue) :
    ExSim (fun (m mF : MapValue) => mF = { base := m.base.map (renameLoc ρ) })
      (valueAsMap v) (valueAsMap (renameValue ρ v)) := by
  cases v <;> simp only [renameValue] <;>
    first
    | exact ExSim.stuck'
    | exact ExSim.ok rfl

/-! ## `validateSlice` (stuck-only) as a full `ExSim` fact -/

theorem exsim_guard_SS {α β : Type} {R : α → β → Prop} {c : Prop}
    [Decidable c] {m₁ m₂ : String} {x : Except GoError α}
    {y : Except GoError β} (h : ExSim R x y) :
    ExSim R
      (if c then (stuck m₁ : Except GoError PUnit) >>= fun _ => x
        else pure PUnit.unit >>= fun _ => x)
      (if c then (stuck m₂ : Except GoError PUnit) >>= fun _ => y
        else pure PUnit.unit >>= fun _ => y) := by
  by_cases hc : c
  · rw [if_pos hc, if_pos hc]
    exact ExSim.stuck'
  · rw [if_neg hc, if_neg hc]
    exact h

theorem validateSlice_exsim_SS (ρ : Nat → Nat) (s : SliceValue) :
    ExSim (fun _ _ => True) (validateSlice s)
      (validateSlice { s with base := s.base.map (renameLoc ρ) }) := by
  obtain ⟨b, o, ln, cp⟩ := s
  simp only [validateSlice]
  refine exsim_guard_SS ?_
  cases b with
  | none =>
      simp only [Option.map_none]
      exact ExSim.ite_congr (fun _ => ExSim.ok trivial) (fun _ => ExSim.stuck')
  | some l => exact ExSim.ok trivial

/-! ## `sliceVisibleValues` -/

theorem sliceVisibleValues_sim_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (sl : SliceValue) :
    ExSim (fun vs vsF => vsF = (renameValueList ρ vs.toList).toArray)
      (sliceVisibleValues σ sl)
      (sliceVisibleValues σF { sl with base := sl.base.map (renameLoc ρ) }) := by
  obtain ⟨base, off, len, cap⟩ := sl
  simp only [sliceVisibleValues]
  refine ExSim.bind (validateSlice_exsim_SS ρ ⟨base, off, len, cap⟩) ?_
  intro _ _ _
  refine ExSim.bind
    (R := fun vs vsF => vsF = (renameValueList ρ vs.toList).toArray) ?_ ?_
  · rw [Std.Legacy.Range.forIn_eq_forIn_range',
      Std.Legacy.Range.forIn_eq_forIn_range']
    refine forIn_sim_same (by simp [renameValueList]) ?_
    intro i _ x y hxy
    subst hxy
    refine ExSim.bind (sliceIndexLoc_sim ρ ⟨base, off, len, cap⟩ (Int.ofNat i)) ?_
    intro l lF hlF
    subst hlF
    refine ExSim.bind (loadLoc_sim hS.toFrameSim l) ?_
    intro v vF hvF
    subst hvF
    exact ExSim.ok (renArr_push_SS x v)
  · intro vs vsF h
    exact ExSim.ok h

/-! ## `mapLookupValue` -/

theorem mapLookupValue_sim_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (hinj : ∀ {x y : Nat}, ρ x = ρ y → x = y)
    (m : MapValue) (key : GoValue) (kt vt : Ty) :
    ExSim (fun p pF => pF = (renameValue ρ p.1, p.2))
      (mapLookupValue σ m key kt vt)
      (mapLookupValue σF { base := m.base.map (renameLoc ρ) }
        (renameValue ρ key) kt vt) := by
  simp only [mapLookupValue]
  refine ExSim.bind (mapEntries_sim hS.toFrameSim m) ?_
  intro o oF hoF
  subst hoF
  cases o with
  | none =>
      simp only [Option.map_none]
      refine ExSim.bind (checkKeyHashable_sim (ρ := ρ) hS.types_eq key false false) ?_
      intro _ _ _
      refine ExSim.bind (defaultValue_sim (ρ := ρ) hS.types_eq vt) ?_
      intro d dF hdF
      subst hdF
      exact ExSim.ok rfl
  | some p =>
      obtain ⟨bl, entries⟩ := p
      simp only [Option.map_some]
      refine ExSim.bind (mapEntryIndex?_sim hinj hS.types_eq kt entries key false) ?_
      intro i i' hi
      subst hi
      cases i with
      | none =>
          refine ExSim.bind (defaultValue_sim (ρ := ρ) hS.types_eq vt) ?_
          intro d dF hdF
          subst hdF
          exact ExSim.ok rfl
      | some idx =>
          dsimp only
          rw [renEntries_getElem?]
          cases hidx : entries[idx]? with
          | none => exact ExSim.stuck'
          | some e =>
              obtain ⟨k1, v1⟩ := e
              simp only [Option.map_some]
              exact ExSim.ok rfl

/-! ## `indexTargetLoc` -/

theorem indexTargetLoc_sim_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (b i : GoValue) :
    ExSim (fun l lF => lF = renameLoc ρ l)
      (indexTargetLoc σ b i)
      (indexTargetLoc σF (renameValue ρ b) (renameValue ρ i)) := by
  simp only [indexTargetLoc]
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro n n' hn
  subst hn
  cases b <;> simp only [renameValue]
  case slice sl => exact sliceIndexLoc_sim ρ sl n
  case nil => exact ExSim.panic
  case addr l =>
      refine ExSim.bind (loadLoc_sim hS.toFrameSim l) ?_
      intro v vF hvF
      subst hvF
      cases v <;> simp only [renameValue]
      case array vals =>
          rw [arrayIndexNat_ren]
          refine ExSim.bind (ExSim.refl _) ?_
          intro _ _ h
          subst h
          exact ExSim.ok rfl
      case slice sl => exact sliceIndexLoc_sim ρ sl n
      all_goals exact ExSim.stuck'
  all_goals exact ExSim.stuck'

/-! ## `applySlice` -/

theorem applySlice_sim_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (b : GoValue) (lo hi : Int) (m : Option Int) :
    ExSim (fun (r rF : GoValue × ExecState) =>
        rF.1 = renameValue ρ r.1 ∧ FrameSimS ρ na₀ na fr r.2 rF.2)
      (applySlice σ b lo hi m)
      (applySlice σF (renameValue ρ b) lo hi m) := by
  simp only [applySlice]
  cases b <;> simp only [renameValue]
  case string gs => exact scalar_pair_SS hS (stringSlice_ren_sim ρ gs lo hi m)
  case slice sl => exact scalar_pair_SS hS (sliceFromSlice_sim ρ sl lo hi m)
  case addr l =>
      refine ExSim.bind (loadLoc_sim hS.toFrameSim l) ?_
      intro v vF hvF
      subst hvF
      cases v <;> simp only [renameValue]
      case array vals =>
          rw [renArr_size_SS]
          exact scalar_pair_SS hS (sliceFromArray_sim ρ l vals.size lo hi m)
      case slice sl => exact scalar_pair_SS hS (sliceFromSlice_sim ρ sl lo hi m)
      all_goals exact ExSim.stuck'
  case array vals => exact ExSim.unsupported'
  all_goals exact ExSim.stuck'

/-! ## Scalar operator helpers: panic-freedom, ok-shape, transfer

The int/float operator helpers error only with `stuck`/`unsupported`
and produce loc-free (`.int`/`.float`) ok-results, so their `ExSim`
facts follow from the `Compare.lean` ok-transfers with no value-shape
case explosion. -/

theorem valueAsIntValue_noPanic_SS (v : GoValue) :
    NoPanic (valueAsIntValue v) := by
  cases v <;> first | exact NoPanic.pure' | exact NoPanic.stuck'

theorem intBinaryResult_noPanic_SS (nm : String) (op : Int → Int → Int)
    (l r : GoValue) : NoPanic (intBinaryResult nm op l r) := by
  unfold intBinaryResult
  refine NoPanic.bind (valueAsIntValue_noPanic_SS l) fun p => ?_
  split
  refine NoPanic.bind (valueAsIntValue_noPanic_SS r) fun q => ?_
  split
  split
  · exact NoPanic.bind NoPanic.pure' fun _ => NoPanic.pure'
  · exact NoPanic.bind NoPanic.stuck' fun _ => NoPanic.pure'

theorem intBinaryResult_ok_SS (ρ : Nat → Nat) {nm : String}
    {op : Int → Int → Int} {l r v : GoValue}
    (h : intBinaryResult nm op l r = .ok v) : renameValue ρ v = v := by
  simp only [intBinaryResult, bind_eq_ok, pure_eq_ok] at h
  obtain ⟨⟨lv, lk⟩, -, ⟨rv, rk⟩, -, h⟩ := h
  split at h
  · simp only [Bind.bind, Except.bind, Except.ok.injEq] at h
    subst h
    simp [renameValue]
  · simp [Bind.bind, Except.bind] at h

theorem intBinaryResult_sim_SS (ρ : Nat → Nat) {nm : String}
    {op : Int → Int → Int} {l r lF rF : GoValue}
    (hl : lF = renameValue ρ l) (hr : rF = renameValue ρ r) :
    ExSim (fun a b => b = renameValue ρ a)
      (intBinaryResult nm op l r) (intBinaryResult nm op lF rF) := by
  subst hl hr
  refine exSim_of_ren (intBinaryResult_noPanic_SS nm op l r) ?_
  intro a ha
  rw [intBinaryResult_ren ρ ha, intBinaryResult_ok_SS ρ ha]

theorem intKindBitWidth_noPanic_SS (nm : String) (k : IntKind) :
    NoPanic (intKindBitWidth nm k) := by
  unfold intKindBitWidth
  split
  · exact NoPanic.pure'
  · exact NoPanic.unsupported'

theorem intKindUnsignedNat_noPanic_SS (k : IntKind) (v : Int) :
    NoPanic (intKindUnsignedNat k v) := by
  unfold intKindUnsignedNat
  exact NoPanic.bind (intKindBitWidth_noPanic_SS _ _) fun _ => NoPanic.pure'

theorem intBitwiseBinaryResult_noPanic_SS (nm : String)
    (op : Nat → Nat → Nat) (l r : GoValue) :
    NoPanic (intBitwiseBinaryResult nm op l r) := by
  unfold intBitwiseBinaryResult
  refine NoPanic.bind (valueAsIntValue_noPanic_SS l) fun p => ?_
  split
  refine NoPanic.bind (valueAsIntValue_noPanic_SS r) fun q => ?_
  split
  split
  · refine NoPanic.bind NoPanic.pure' fun kind => ?_
    refine NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => ?_
    exact NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => NoPanic.pure'
  · refine NoPanic.bind NoPanic.stuck' fun kind => ?_
    refine NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => ?_
    exact NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => NoPanic.pure'

theorem intBitwiseBinaryResult_ok_SS (ρ : Nat → Nat) {nm : String}
    {op : Nat → Nat → Nat} {l r v : GoValue}
    (h : intBitwiseBinaryResult nm op l r = .ok v) : renameValue ρ v = v := by
  simp only [intBitwiseBinaryResult, bind_eq_ok, pure_eq_ok] at h
  obtain ⟨⟨lv, lk⟩, -, ⟨rv, rk⟩, -, h⟩ := h
  split at h
  · simp only [bind_eq_ok, Except.ok.injEq] at h
    obtain ⟨a, -, lb, -, rb, -, h⟩ := h
    subst h
    simp [renameValue]
  · simp [Bind.bind, Except.bind] at h

theorem intBitwiseBinaryResult_sim_SS (ρ : Nat → Nat) {nm : String}
    {op : Nat → Nat → Nat} (l r : GoValue) :
    ExSim (fun a b => b = renameValue ρ a)
      (intBitwiseBinaryResult nm op l r)
      (intBitwiseBinaryResult nm op (renameValue ρ l) (renameValue ρ r)) := by
  refine exSim_of_ren (intBitwiseBinaryResult_noPanic_SS nm op l r) ?_
  intro a ha
  rw [intBitwiseBinaryResult_ren ρ ha, intBitwiseBinaryResult_ok_SS ρ ha]

theorem intBitClearResult_noPanic_SS (l r : GoValue) :
    NoPanic (intBitClearResult l r) := by
  unfold intBitClearResult
  refine NoPanic.bind (valueAsIntValue_noPanic_SS l) fun p => ?_
  split
  refine NoPanic.bind (valueAsIntValue_noPanic_SS r) fun q => ?_
  split
  split
  · refine NoPanic.bind NoPanic.pure' fun kind => ?_
    refine NoPanic.bind (intKindBitWidth_noPanic_SS _ _) fun _ => ?_
    refine NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => ?_
    exact NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => NoPanic.pure'
  · refine NoPanic.bind NoPanic.stuck' fun kind => ?_
    refine NoPanic.bind (intKindBitWidth_noPanic_SS _ _) fun _ => ?_
    refine NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => ?_
    exact NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => NoPanic.pure'

theorem intBitClearResult_ok_SS (ρ : Nat → Nat) {l r v : GoValue}
    (h : intBitClearResult l r = .ok v) : renameValue ρ v = v := by
  simp only [intBitClearResult, bind_eq_ok, pure_eq_ok] at h
  obtain ⟨⟨lv, lk⟩, -, ⟨rv, rk⟩, -, h⟩ := h
  split at h
  · simp only [bind_eq_ok, Except.ok.injEq] at h
    obtain ⟨a, -, bits, -, lb, -, rb, -, h⟩ := h
    subst h
    simp [renameValue]
  · simp [Bind.bind, Except.bind] at h

theorem intBitClearResult_sim_SS (ρ : Nat → Nat) (l r : GoValue) :
    ExSim (fun a b => b = renameValue ρ a)
      (intBitClearResult l r)
      (intBitClearResult (renameValue ρ l) (renameValue ρ r)) := by
  refine exSim_of_ren (intBitClearResult_noPanic_SS l r) ?_
  intro a ha
  rw [intBitClearResult_ren ρ ha, intBitClearResult_ok_SS ρ ha]

theorem intBitNegResult_noPanic_SS (v : GoValue) :
    NoPanic (intBitNegResult v) := by
  unfold intBitNegResult
  refine NoPanic.bind (valueAsIntValue_noPanic_SS v) fun p => ?_
  split
  refine NoPanic.bind (intKindBitWidth_noPanic_SS _ _) fun _ => ?_
  exact NoPanic.bind (intKindUnsignedNat_noPanic_SS _ _) fun _ => NoPanic.pure'

theorem intBitNegResult_ok_SS (ρ : Nat → Nat) {x v : GoValue}
    (h : intBitNegResult x = .ok v) : renameValue ρ v = v := by
  simp only [intBitNegResult, bind_eq_ok, pure_eq_ok, Except.ok.injEq] at h
  obtain ⟨⟨v0, k0⟩, -, bits, -, vb, -, h⟩ := h
  subst h
  simp [renameValue]

theorem intBitNegResult_sim_SS (ρ : Nat → Nat) (v : GoValue) :
    ExSim (fun a b => b = renameValue ρ a)
      (intBitNegResult v) (intBitNegResult (renameValue ρ v)) := by
  refine exSim_of_ren (intBitNegResult_noPanic_SS v) ?_
  intro a ha
  rw [intBitNegResult_ren ρ ha, intBitNegResult_ok_SS ρ ha]

theorem floatBinaryResult_ok_SS (ρ : Nat → Nat) {nm : String}
    {op64 op32 : Nat → Nat → Nat} {l r v : GoValue}
    (h : floatBinaryResult nm op64 op32 l r = .ok v) : renameValue ρ v = v := by
  unfold floatBinaryResult at h
  split at h
  · split at h
    · split at h <;>
        (simp only [pure_eq_ok, Except.ok.injEq] at h
         subst h
         simp [renameValue])
    · simp at h
  · simp at h

/-! ## Ordering comparisons -/

theorem floatCompareResult_noPanic_SS (nm : String)
    (c64 c32 : Nat → Nat → Bool) (l r : GoValue) :
    NoPanic (floatCompareResult nm c64 c32 l r) := by
  unfold floatCompareResult
  split
  · split
    · split <;> exact NoPanic.pure'
    · exact NoPanic.stuck'
  · exact NoPanic.stuck'

theorem valueLess_noPanic_SS (l r : GoValue) : NoPanic (valueLess l r) := by
  unfold valueLess
  split
  · exact NoPanic.pure'
  · exact floatCompareResult_noPanic_SS _ _ _ _ _
  · exact NoPanic.pure'
  · exact NoPanic.stuck'

theorem valueAtMost_noPanic_SS (l r : GoValue) :
    NoPanic (valueAtMost l r) := by
  unfold valueAtMost
  split
  · exact NoPanic.pure'
  · exact floatCompareResult_noPanic_SS _ _ _ _ _
  · exact NoPanic.pure'
  · exact NoPanic.stuck'

theorem valueGreater_noPanic_SS (l r : GoValue) :
    NoPanic (valueGreater l r) := by
  unfold valueGreater
  split
  · exact NoPanic.pure'
  · exact floatCompareResult_noPanic_SS _ _ _ _ _
  · exact NoPanic.pure'
  · exact NoPanic.stuck'

theorem valueAtLeast_noPanic_SS (l r : GoValue) :
    NoPanic (valueAtLeast l r) := by
  unfold valueAtLeast
  split
  · exact NoPanic.pure'
  · exact floatCompareResult_noPanic_SS _ _ _ _ _
  · exact NoPanic.pure'
  · exact NoPanic.stuck'

theorem valueLess_sim_SS (ρ : Nat → Nat) {l r lF rF : GoValue}
    (hl : lF = renameValue ρ l) (hr : rF = renameValue ρ r) :
    ExSim Eq (valueLess l r) (valueLess lF rF) := by
  subst hl hr
  exact exsim_eq_of_ren_SS (valueLess_noPanic_SS l r) fun _ hb => valueLess_ren ρ hb

theorem valueAtMost_sim_SS (ρ : Nat → Nat) (l r : GoValue) :
    ExSim Eq (valueAtMost l r)
      (valueAtMost (renameValue ρ l) (renameValue ρ r)) :=
  exsim_eq_of_ren_SS (valueAtMost_noPanic_SS l r) fun _ hb => valueAtMost_ren ρ hb

theorem valueGreater_sim_SS (ρ : Nat → Nat) (l r : GoValue) :
    ExSim Eq (valueGreater l r)
      (valueGreater (renameValue ρ l) (renameValue ρ r)) :=
  exsim_eq_of_ren_SS (valueGreater_noPanic_SS l r) fun _ hb => valueGreater_ren ρ hb

theorem valueAtLeast_sim_SS (ρ : Nat → Nat) (l r : GoValue) :
    ExSim Eq (valueAtLeast l r)
      (valueAtLeast (renameValue ρ l) (renameValue ρ r)) :=
  exsim_eq_of_ren_SS (valueAtLeast_noPanic_SS l r) fun _ hb => valueAtLeast_ren ρ hb

/-! ## Convert result transfer (ok via the rename lemma; the
slice→array length-check PANIC — triage L2a, 2026-08-19 — via
`convertValueToTy_panic_ren`; the `_noPanic` lemma this section
carried is retired: the length check made it false, and the arm proof
now transfers each result class directly.) -/

/-! ## Panic-freedom of `defaultValue` -/

theorem defaultFieldsWith_noPanic_SS {f : Ty → Except GoError GoValue}
    (hf : ∀ ty, NoPanic (f ty)) :
    ∀ fds : List FieldDef, NoPanic (defaultFieldsWith f fds) := by
  intro fds
  induction fds with
  | nil => exact NoPanic.pure'
  | cons fd rest ih =>
      simp only [defaultFieldsWith]
      exact NoPanic.bind (hf fd.typ) fun _ =>
        NoPanic.bind ih fun _ => NoPanic.pure'

theorem defaultValueFuel_noPanic_SS (σ₀ : ExecState) :
    ∀ (fuel : Nat) (ty : Ty), NoPanic (defaultValueFuel fuel σ₀ ty) := by
  intro fuel
  induction fuel with
  | zero => intro ty; exact NoPanic.unsupported'
  | succ n ih =>
      intro ty
      cases ty
      case array len elem =>
          simp only [defaultValueFuel]
          refine NoPanic.ite' NoPanic.pure' ?_
          exact NoPanic.bind NoPanic.pure' fun _ =>
            NoPanic.bind (ih elem) fun _ => NoPanic.pure'
      case defined name =>
          simp only [defaultValueFuel]
          split
          · exact NoPanic.map (defaultFieldsWith_noPanic_SS (fun ty => ih ty) _)
          · exact ih _
          · exact ih _
          · exact NoPanic.unsupported'
          · exact NoPanic.unsupported'
          · exact NoPanic.unsupported'
      all_goals first | exact NoPanic.pure' | exact NoPanic.unsupported'

theorem defaultValue_noPanic_SS (σ₀ : ExecState) (ty : Ty) :
    NoPanic (defaultValue σ₀ ty) := by
  rw [defaultValue]
  exact defaultValueFuel_noPanic_SS σ₀ _ ty

/-! ## Panic-freedom of the struct/array literal builders -/

theorem buildStructFields_noPanic_SS (σ₀ : ExecState) :
    ∀ (fds : List FieldDef) (vs : List GoValue),
      NoPanic (buildStructFields σ₀ fds vs) := by
  intro fds
  induction fds with
  | nil => intro vs; exact NoPanic.pure'
  | cons fd rest ih =>
      intro vs
      cases vs with
      | nil => exact NoPanic.pure'
      | cons v vrest =>
          simp only [buildStructFields]
          exact NoPanic.bind (normalizeValueForTy_noPanic σ₀ _ _) fun _ =>
            NoPanic.bind (ih vrest) fun _ => NoPanic.pure'

theorem buildStructValueFuel_noPanic_SS (σ₀ : ExecState) :
    ∀ (fuel : Nat) (ty : Ty) (args : Array GoValue),
      NoPanic (buildStructValueFuel fuel σ₀ ty args) := by
  intro fuel
  induction fuel with
  | zero =>
      intro ty args
      rw [buildStructValueFuel.eq_def]
      split <;>
        first
        | exact NoPanic.unsupported'
        | (rename_i heq; exact absurd heq.symm (Nat.succ_ne_zero _))
  | succ n ih =>
      intro ty args
      rw [buildStructValueFuel.eq_def]
      split <;>
        first
        | exact NoPanic.unsupported'
        | (rename_i heq; exact absurd heq (Nat.succ_ne_zero _))
        | skip
      all_goals (
        rename_i heq
        injection heq with hfe
        subst hfe
        split
        · -- struct: size guard (join point), then the field builder
          split
          · exact NoPanic.bind NoPanic.stuck' fun _ =>
              NoPanic.map (buildStructFields_noPanic_SS _ _ _)
          · exact NoPanic.bind NoPanic.pure' fun _ =>
              NoPanic.map (buildStructFields_noPanic_SS _ _ _)
        · exact ih _ _
        · exact NoPanic.unsupported'
        · exact NoPanic.unsupported'
        · exact NoPanic.unsupported'
        · exact NoPanic.unsupported')

theorem buildStructValue_noPanic_SS (σ₀ : ExecState) (ty : Ty)
    (args : Array GoValue) : NoPanic (buildStructValue σ₀ ty args) := by
  rw [buildStructValue]
  exact buildStructValueFuel_noPanic_SS σ₀ _ ty args

/-- Panic-decomposition of `bind` (the error-side sibling of
`bind_eq_ok`, stated for exactly the panic error). -/
theorem bind_eq_panic_SS {α β : Type} {x : Except GoError α}
    {f : α → Except GoError β} {m : String} :
    (x >>= f = .error (.panic m))
      ↔ (x = .error (.panic m) ∨ ∃ a, x = .ok a ∧ f a = .error (.panic m)) := by
  cases x with
  | ok a => simp [Bind.bind, Except.bind]
  | error e => simp [Bind.bind, Except.bind]

/-- A panicking list-`forIn` has a panicking body step. -/
theorem forIn_panic_SS {α β : Type}
    {f : α → β → Except GoError (ForInStep β)} {m : String} :
    ∀ {l : List α} {b : β}, forIn l b f = .error (.panic m) →
      ∃ a ∈ l, ∃ x, f a x = .error (.panic m) := by
  intro l
  induction l with
  | nil =>
      intro b h
      rw [List.forIn_nil] at h
      simp [pure, Except.pure] at h
  | cons a as ih =>
      intro b h
      rw [List.forIn_cons] at h
      rcases bind_eq_panic_SS.mp h with h1 | ⟨r, -, h2⟩
      · exact ⟨a, by simp, b, h1⟩
      · cases r with
        | done x => simp [pure, Except.pure] at h2
        | yield x =>
            obtain ⟨a', ha', x', hx'⟩ := ih h2
            exact ⟨a', by simp [ha'], x', hx'⟩

theorem buildArrayValue_noPanic_SS (σ₀ : ExecState) (len : Nat)
    (elem : Ty) (args : Array (Int × GoValue)) :
    NoPanic (buildArrayValue σ₀ len elem args) := by
  intro m h
  unfold buildArrayValue at h
  simp only [bind_eq_panic_SS] at h
  rcases h with h | ⟨vs₁, -, h⟩
  · -- default-fill loop
    rw [Std.Legacy.Range.forIn_eq_forIn_range'] at h
    obtain ⟨i, -, x, hx⟩ := forIn_panic_SS h
    simp only [bind_eq_panic_SS] at hx
    rcases hx with h1 | ⟨d, -, h2⟩
    · exact defaultValue_noPanic_SS σ₀ elem m h1
    · simp [bind_eq_panic_SS] at h2
  · -- keyed-entry loop (the final pure is pruned by the outer simp)
    rw [← Array.forIn_toList] at h
    rcases h with h | ⟨st, -, hfin⟩
    · obtain ⟨p, -, x, hx⟩ := forIn_panic_SS h
      obtain ⟨key, value⟩ := p
      split at hx
      · simp [bind_eq_panic_SS] at hx
      · simp only [bind_eq_panic_SS, pure_eq_ok, Except.ok.injEq,
          reduceCtorEq, false_or, exists_eq_left'] at hx
        split at hx
        · simp [bind_eq_panic_SS] at hx
        · simp only [bind_eq_panic_SS, pure_eq_ok, Except.ok.injEq,
            reduceCtorEq, false_or, exists_eq_left'] at hx
          split at hx
          · simp [bind_eq_panic_SS] at hx
            rcases hx with h1 | ⟨nv, -, h2⟩
            · exact normalizeValueForTy_noPanic σ₀ elem value m h1
            · exact coerceStoredValue_noPanic _ _ m h2
          · simp [bind_eq_panic_SS] at hx
    · simp [pure, Except.pure] at hfin

/-! ## `typeAssertValue` simulation and ok-false inversion -/

theorem typeAssertValue_sim_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) (ty : Ty) :
    ExSim (fun (p pF : GoValue × Bool) => pF = (renameValue ρ p.1, p.2))
      (typeAssertValue σ v ty)
      (typeAssertValue σF (renameValue ρ v) ty) := by
  simp only [typeAssertValue]
  refine ExSim.bind (defaultValue_sim (ρ := ρ) hS.types_eq ty) ?_
  intro d dF hdF
  subst hdF
  cases v <;> simp only [renameValue]
  case nil => exact ExSim.ok rfl
  case interface dynTy inner =>
      rw [resolveDefinedAliases_congr hS.types_eq]
      cases hres : resolveDefinedAliases σ ty
      case interface iname =>
          dsimp only
          rw [dynamicImplementsInterface_congr hS.types_eq hS.methods_eq
            hS.funcs_eq hS.methodSets_eq]
          refine ExSim.bind (ExSim.refl _) ?_
          intro b b' hb
          subst hb
          cases b
          · exact ExSim.ok rfl
          · exact ExSim.ok (by simp only [renameValue])
      all_goals (
        rw [canonicalTy_congr hS.types_eq]
        exact ExSim.ite_congr (fun _ => ExSim.ok rfl) (fun _ => ExSim.ok rfl))
  all_goals exact ExSim.unsupported'

theorem typeAssertValue_ok_false_SS {σ₀ : ExecState} {v : GoValue}
    {ty : Ty} {r : GoValue}
    (h : typeAssertValue σ₀ v ty = .ok (r, false)) :
    v = .nil ∨ ∃ t i, v = .interface t i := by
  cases v
  case nil => exact Or.inl rfl
  case interface t i => exact Or.inr ⟨t, i, rfl⟩
  all_goals (
    exfalso
    simp only [typeAssertValue, bind_eq_ok] at h
    obtain ⟨d, -, hbad⟩ := h
    simp at hbad)

/-! ## The `applyStrictOp` arms -/

/-- The result relation of `applyStrictOp_sim_SS`, locally named. -/
abbrev PairSim_SS (ρ : Nat → Nat) (na₀ na : Nat) (fr : Heap) :
    GoValue × ExecState → GoValue × ExecState → Prop :=
  fun r rF => rF.1 = renameValue ρ r.1 ∧ FrameSimS ρ na₀ na fr r.2 rF.2

set_option maxHeartbeats 1600000 in
theorem arm_add_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .add [l, r])
      (applyStrictOp σF .add [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  cases l
  case int a ka =>
      cases r <;>
        first
        | exact ExSim.stuck'
        | (simp only [renameValue]
           exact scalar_pair_SS hS (exsim_ren_self_SS ρ fun v hv => intBinaryResult_ok_SS ρ hv))
  case float lb lk =>
      cases r <;>
        first
        | exact ExSim.stuck'
        | (simp only [renameValue]
           exact scalar_pair_SS hS (exsim_ren_self_SS ρ fun v hv => floatBinaryResult_ok_SS ρ hv))
  case string sa =>
      cases r <;>
        first
        | exact ExSim.stuck'
        | (simp only [renameValue]
           exact ExSim.ok ⟨by simp only [renameValue], hS⟩)
  all_goals exact ExSim.stuck'

set_option maxHeartbeats 1600000 in
theorem arm_sub_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .sub [l, r])
      (applyStrictOp σF .sub [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  cases l
  case float lb lk =>
      cases r <;> simp only [renameValue] <;>
        first
        | exact scalar_pair_SS hS (exsim_ren_self_SS ρ fun v hv => floatBinaryResult_ok_SS ρ hv)
        | exact scalar_pair_SS hS
            (intBinaryResult_sim_SS ρ (by simp [renameValue]) (by simp [renameValue]))
  all_goals (
    simp only [renameValue]
    exact scalar_pair_SS hS
      (intBinaryResult_sim_SS ρ (by simp [renameValue]) rfl))

set_option maxHeartbeats 1600000 in
theorem arm_mul_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .mul [l, r])
      (applyStrictOp σF .mul [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  cases l
  case float lb lk =>
      cases r <;> simp only [renameValue] <;>
        first
        | exact scalar_pair_SS hS (exsim_ren_self_SS ρ fun v hv => floatBinaryResult_ok_SS ρ hv)
        | exact scalar_pair_SS hS
            (intBinaryResult_sim_SS ρ (by simp [renameValue]) (by simp [renameValue]))
  all_goals (
    simp only [renameValue]
    exact scalar_pair_SS hS
      (intBinaryResult_sim_SS ρ (by simp [renameValue]) rfl))

set_option maxHeartbeats 1600000 in
theorem arm_div_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .div [l, r])
      (applyStrictOp σF .div [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  cases l <;>
    first
    | -- non-float rows: the default arm with `r` general
      (simp only [renameValue]
       refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
       intro d d' hd
       subst hd
       by_cases hz : (d == 0) = true
       · rw [if_pos hz, if_pos hz]
         exact ExSim.panic
       · rw [if_neg hz, if_neg hz]
         simp only [pure_bind]
         exact scalar_pair_SS hS
           (intBinaryResult_sim_SS ρ (by simp [renameValue]) rfl))
    | -- float row: split on r for the float×float arm
      (cases r <;> simp only [renameValue] <;>
        first
        | exact scalar_pair_SS hS (exsim_ren_self_SS ρ fun v hv => floatBinaryResult_ok_SS ρ hv)
        | (refine ExSim.bind (valueAsInt_sim_SS ρ (by simp [renameValue])) ?_
           intro d d' hd
           subst hd
           by_cases hz : (d == 0) = true
           · rw [if_pos hz, if_pos hz]
             exact ExSim.panic
           · rw [if_neg hz, if_neg hz]
             simp only [pure_bind]
             exact scalar_pair_SS hS
               (intBinaryResult_sim_SS ρ (by simp [renameValue]) (by simp [renameValue]))))

theorem arm_mod_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .mod [l, r])
      (applyStrictOp σF .mod [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro d d' hd
  subst hd
  by_cases hz : (d == 0) = true
  · rw [if_pos hz, if_pos hz]
    exact ExSim.panic
  · rw [if_neg hz, if_neg hz]
    simp only [pure_bind]
    exact scalar_pair_SS hS (intBinaryResult_sim_SS ρ rfl rfl)

theorem arm_shiftLeft_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .shiftLeft [l, r])
      (applyStrictOp σF .shiftLeft [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  exact scalar_pair_SS hS (intShiftLeftResult_sim ρ l r)

theorem arm_shiftRight_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .shiftRight [l, r])
      (applyStrictOp σF .shiftRight [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  exact scalar_pair_SS hS (intShiftRightResult_sim ρ l r)

theorem arm_bitAnd_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .bitAnd [l, r])
      (applyStrictOp σF .bitAnd [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  exact scalar_pair_SS hS (intBitwiseBinaryResult_sim_SS ρ l r)

theorem arm_bitOr_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .bitOr [l, r])
      (applyStrictOp σF .bitOr [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  exact scalar_pair_SS hS (intBitwiseBinaryResult_sim_SS ρ l r)

theorem arm_bitXor_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .bitXor [l, r])
      (applyStrictOp σF .bitXor [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  exact scalar_pair_SS hS (intBitwiseBinaryResult_sim_SS ρ l r)

theorem arm_bitClear_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .bitClear [l, r])
      (applyStrictOp σF .bitClear [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  exact scalar_pair_SS hS (intBitClearResult_sim_SS ρ l r)

theorem arm_bitNeg_SS (hS : FrameSimS ρ na₀ na fr σ σF) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .bitNeg [v])
      (applyStrictOp σF .bitNeg [renameValue ρ v]) := by
  simp only [applyStrictOp]
  exact scalar_pair_SS hS (intBitNegResult_sim_SS ρ v)

theorem arm_neg_SS (hS : FrameSimS ρ na₀ na fr σ σF) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .neg [v])
      (applyStrictOp σF .neg [renameValue ρ v]) := by
  simp only [applyStrictOp]
  cases v <;> simp only [renameValue] <;>
    first
    | exact ExSim.stuck'
    | exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_not_SS (hS : FrameSimS ρ na₀ na fr σ σF) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .not [v])
      (applyStrictOp σF .not [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsBool_sim_SS ρ v) ?_
  intro b b' hb
  subst hb
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_floatLit_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (num : Int) (den : Nat) (kind : FloatKind) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.floatLit num den kind) [])
      (applyStrictOp σF (.floatLit num den kind) []) := by
  simp only [applyStrictOp]
  by_cases hd : (den == 0) = true
  · rw [if_pos hd, if_pos hd]
    exact ExSim.stuck'
  · rw [if_neg hd, if_neg hd]
    exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_eq_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (hinj : ∀ {x y : Nat}, ρ x = ρ y → x = y) (ty : Ty) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.eqCmp ty) [l, r])
      (applyStrictOp σF (.eqCmp ty) [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueEq_sim hinj hS.types_eq ty l r) ?_
  intro b b' hb
  subst hb
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_neq_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (hinj : ∀ {x y : Nat}, ρ x = ρ y → x = y) (ty : Ty) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.neqCmp ty) [l, r])
      (applyStrictOp σF (.neqCmp ty) [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueEq_sim hinj hS.types_eq ty l r) ?_
  intro b b' hb
  subst hb
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_atMost_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .atMostCmp [l, r])
      (applyStrictOp σF .atMostCmp [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAtMost_sim_SS ρ l r) ?_
  intro b b' hb
  subst hb
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_atLeast_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .atLeastCmp [l, r])
      (applyStrictOp σF .atLeastCmp [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAtLeast_sim_SS ρ l r) ?_
  intro b b' hb
  subst hb
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_less_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .lessCmp [l, r])
      (applyStrictOp σF .lessCmp [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueLess_sim_SS ρ rfl rfl) ?_
  intro b b' hb
  subst hb
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_greater_SS (hS : FrameSimS ρ na₀ na fr σ σF) (l r : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .greaterCmp [l, r])
      (applyStrictOp σF .greaterCmp [renameValue ρ l, renameValue ρ r]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueGreater_sim_SS ρ l r) ?_
  intro b b' hb
  subst hb
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_convert_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (ty : Ty) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.convert ty) [v])
      (applyStrictOp σF (.convert ty) [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine scalar_pair_SS hS ?_
  cases hx : convertValueToTy σ ty v with
  | ok a =>
      rw [convertValueToTy_ren ρ hS.types_eq hx]
      exact ExSim.ok rfl
  | error e =>
      cases e with
      | panic m =>
          -- The slice→array(-pointer) length check (triage L2a): the
          -- message embeds `len` and the target length only, both
          -- rename-invariant. `ExSim` at a canonical panic IS the
          -- framed-side equation, which is the transfer lemma verbatim.
          exact convertValueToTy_panic_ren ρ hS.types_eq hx
      | _ => trivial

theorem arm_bytesFromString_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .bytesFromString [v])
      (applyStrictOp σF .bytesFromString [renameValue ρ v]) := by
  simp only [applyStrictOp]
  cases v <;> simp only [renameValue]
  case string gs =>
      have hb : renameValue ρ
          (.array (gs.bytes.map fun b => GoValue.int (Int.ofNat b.toNat) .uint8))
          = .array (gs.bytes.map fun b => GoValue.int (Int.ofNat b.toNat) .uint8) := by
        simp only [renameValue, renameValueList_eq_map, Array.toList_map,
          List.map_map]
        congr 1
        apply Array.toList_inj.mp
        simp [Function.comp_def, renameValue]
      have h1 : (σF.alloc
            (.array (gs.bytes.map fun b => GoValue.int (Int.ofNat b.toNat) .uint8))
            (some (.array (gs.bytes.map fun b =>
              GoValue.int (Int.ofNat b.toNat) .uint8).size (.int .uint8)))).1
          = renameLoc ρ ((σ.alloc
            (.array (gs.bytes.map fun b => GoValue.int (Int.ofNat b.toNat) .uint8))
            (some (.array (gs.bytes.map fun b =>
              GoValue.int (Int.ofNat b.toNat) .uint8).size (.int .uint8)))).1) := by
        have h := hS.alloc_fst
          (.array (gs.bytes.map fun b => GoValue.int (Int.ofNat b.toNat) .uint8))
          (some (.array (gs.bytes.map fun b =>
            GoValue.int (Int.ofNat b.toNat) .uint8).size (.int .uint8)))
        rwa [hb] at h
      have h2 : FrameSimS ρ na₀ na fr
          (σ.alloc
            (.array (gs.bytes.map fun b => GoValue.int (Int.ofNat b.toNat) .uint8))
            (some (.array (gs.bytes.map fun b =>
              GoValue.int (Int.ofNat b.toNat) .uint8).size (.int .uint8)))).2
          (σF.alloc
            (.array (gs.bytes.map fun b => GoValue.int (Int.ofNat b.toNat) .uint8))
            (some (.array (gs.bytes.map fun b =>
              GoValue.int (Int.ofNat b.toNat) .uint8).size (.int .uint8)))).2 := by
        have := hS.alloc_snd
          (.array (gs.bytes.map fun b => GoValue.int (Int.ofNat b.toNat) .uint8))
          (some (.array (gs.bytes.map fun b =>
            GoValue.int (Int.ofNat b.toNat) .uint8).size (.int .uint8)))
        rwa [hb] at this
      exact ExSim.ok ⟨by simp only [renameValue, Option.map_some, h1], h2⟩
  all_goals exact ExSim.stuck'

theorem arm_stringFromByteSlice_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .stringFromByteSlice [v])
      (applyStrictOp σF .stringFromByteSlice [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsSlice_sim_SS ρ v) ?_
  intro sl slF hslF
  subst hslF
  refine ExSim.bind (sliceVisibleValues_sim_SS hS sl) ?_
  intro vs vsF hvsF
  subst hvsF
  refine ExSim.bind (R := Eq) ?_ ?_
  · rw [← Array.forIn_toList, ← Array.forIn_toList, List.toList_toArray,
      renameValueList_eq_map]
    refine forIn_sim (t := renameValue ρ) rfl ?_
    intro a _ x y hxy
    subst hxy
    cases a <;>
      first
      | exact ExSim.stuck'
      | (simp only [renameValue]; exact exsim_stepSim_refl_SS _)
  · intro b b' hb
    subst hb
    exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_stringFromRune_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .stringFromRune [v])
      (applyStrictOp σF .stringFromRune [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro n n' hn
  subst hn
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_runesFromString_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .runesFromString [v])
      (applyStrictOp σF .runesFromString [renameValue ρ v]) := by
  simp only [applyStrictOp]
  cases v <;> simp only [renameValue]
  case string gs =>
      have hb : renameValue ρ
          (.array ((runesOfString gs).map fun r => GoValue.int r .int32))
          = .array ((runesOfString gs).map fun r => GoValue.int r .int32) := by
        simp only [renameValue, renameValueList_eq_map, Array.toList_map,
          List.map_map]
        congr 1
        apply Array.toList_inj.mp
        simp [Function.comp_def, renameValue]
      have h1 : (σF.alloc
            (.array ((runesOfString gs).map fun r => GoValue.int r .int32))
            (some (.array ((runesOfString gs).map fun r =>
              GoValue.int r .int32).size (.int .int32)))).1
          = renameLoc ρ ((σ.alloc
            (.array ((runesOfString gs).map fun r => GoValue.int r .int32))
            (some (.array ((runesOfString gs).map fun r =>
              GoValue.int r .int32).size (.int .int32)))).1) := by
        have h := hS.alloc_fst
          (.array ((runesOfString gs).map fun r => GoValue.int r .int32))
          (some (.array ((runesOfString gs).map fun r =>
            GoValue.int r .int32).size (.int .int32)))
        rwa [hb] at h
      have h2 : FrameSimS ρ na₀ na fr
          (σ.alloc
            (.array ((runesOfString gs).map fun r => GoValue.int r .int32))
            (some (.array ((runesOfString gs).map fun r =>
              GoValue.int r .int32).size (.int .int32)))).2
          (σF.alloc
            (.array ((runesOfString gs).map fun r => GoValue.int r .int32))
            (some (.array ((runesOfString gs).map fun r =>
              GoValue.int r .int32).size (.int .int32)))).2 := by
        have := hS.alloc_snd
          (.array ((runesOfString gs).map fun r => GoValue.int r .int32))
          (some (.array ((runesOfString gs).map fun r =>
            GoValue.int r .int32).size (.int .int32)))
        rwa [hb] at this
      exact ExSim.ok ⟨by simp only [renameValue, Option.map_some, h1], h2⟩
  all_goals exact ExSim.stuck'

theorem arm_stringFromRuneSlice_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .stringFromRuneSlice [v])
      (applyStrictOp σF .stringFromRuneSlice [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsSlice_sim_SS ρ v) ?_
  intro sl slF hslF
  subst hslF
  refine ExSim.bind (sliceVisibleValues_sim_SS hS sl) ?_
  intro vs vsF hvsF
  subst hvsF
  refine ExSim.bind (R := Eq) ?_ ?_
  · rw [← Array.forIn_toList, ← Array.forIn_toList, List.toList_toArray,
      renameValueList_eq_map]
    refine forIn_sim (t := renameValue ρ) rfl ?_
    intro a _ x y hxy
    subst hxy
    cases a <;>
      first
      | exact ExSim.stuck'
      | (simp only [renameValue]; exact exsim_stepSim_refl_SS _)
  · intro b b' hb
    subst hb
    exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_deref_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (ty : Ty) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.deref ty) [v])
      (applyStrictOp σF (.deref ty) [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsLoc_sim ρ v) ?_
  intro l lF hl
  subst hl
  exact scalar_pair_SS hS (loadLoc_sim hS.toFrameSim l)

theorem arm_fieldGet_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (tid : TypeId) (fname : String) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.fieldGet tid fname) [v])
      (applyStrictOp σF (.fieldGet tid fname) [renameValue ρ v]) := by
  simp only [applyStrictOp]
  cases v <;> simp only [renameValue]
  case struct actual fields =>
      -- the compat check reads only the types map (triage L7)
      rw [structTagCompatible_congr hS.types_eq]
      by_cases hne :
          (actual != tid && !structTagCompatible σ actual tid) = true
      · rw [if_pos hne]
        exact ExSim.stuck'
      · rw [if_neg hne, if_neg hne]
        simp only [pure_bind]
        rw [structFieldsLookup_ren]
        cases StructFields.lookup fields fname with
        | none => exact ExSim.stuck'
        | some fv => exact ExSim.ok ⟨rfl, hS⟩
  all_goals exact ExSim.stuck'

theorem arm_fieldAddr_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (tid : TypeId) (fname : String) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.fieldAddr tid fname) [v])
      (applyStrictOp σF (.fieldAddr tid fname) [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsLoc_sim ρ v) ?_
  intro l lF hl
  subst hl
  exact ExSim.ok ⟨by simp only [renameValue, renameLoc], hS⟩

theorem arm_addrOfDeref_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .addrOfDeref [v])
      (applyStrictOp σF .addrOfDeref [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsLoc_sim ρ v) ?_
  intro l lF hl
  subst hl
  exact ExSim.ok ⟨by simp only [renameValue, renameLoc], hS⟩

theorem arm_structLit_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (ty : Ty) (vs : List GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.structLit ty) vs)
      (applyStrictOp σF (.structLit ty) (renameValueList ρ vs)) := by
  simp only [applyStrictOp]
  refine scalar_pair_SS hS ?_
  refine exSim_of_ren (buildStructValue_noPanic_SS σ ty vs.toArray) ?_
  intro a ha
  have := buildStructValue_ren ρ hS.types_eq (args := vs.toArray) ha
  simpa [List.toList_toArray] using this

theorem arm_arrayLit_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (n : Nat) (elem : Ty) (keys : List Int) (vs : List GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.arrayLit n elem keys) vs)
      (applyStrictOp σF (.arrayLit n elem keys) (renameValueList ρ vs)) := by
  simp only [applyStrictOp]
  rw [renameValueList_length ρ vs]
  by_cases hlen : (keys.length != vs.length) = true
  · rw [if_pos hlen]
    exact ExSim.stuck'
  · rw [if_neg hlen, if_neg hlen]
    simp only [pure_bind]
    refine scalar_pair_SS hS ?_
    refine exSim_of_ren (buildArrayValue_noPanic_SS σ n elem (keys.zip vs).toArray) ?_
    intro a ha
    have harg : (keys.zip (renameValueList ρ vs)).toArray
        = renameValueEntriesKeyed ρ ((keys.zip vs).toArray) := by
      simp [renameValueEntriesKeyed, renameValueList_eq_map,
        List.zip_map_right, Function.comp_def, Prod.map]
    rw [harg]
    exact buildArrayValue_ren ρ hS.types_eq ha

theorem arm_toInterface_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (tgt dyn : Ty) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.toInterface tgt dyn) [v])
      (applyStrictOp σF (.toInterface tgt dyn) [renameValue ρ v]) := by
  simp only [applyStrictOp]
  rw [canonicalDynamicTy_congr hS.types_eq]
  refine ExSim.bind (ExSim.refl _) ?_
  intro t t' ht
  subst ht
  cases t <;>
    first
    | exact ExSim.ok ⟨rfl, hS⟩
    | exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_typeAssert_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (tgt : Ty) (srcTy : Option Ty) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.typeAssert tgt srcTy) [v])
      (applyStrictOp σF (.typeAssert tgt srcTy) [renameValue ρ v]) := by
  simp only [applyStrictOp]
  refine exSim_bind_ok_SS (typeAssertValue_sim_SS hS v tgt) ?_
  intro p pF hok hrel
  subst hrel
  obtain ⟨r, ok⟩ := p
  cases ok
  case true => exact ExSim.ok ⟨rfl, hS⟩
  case false =>
      have hshape := typeAssertValue_ok_false_SS hok
      rw [resolveDefinedAliases_congr hS.types_eq]
      rcases hshape with rfl | ⟨t, i, rfl⟩
      · -- nil operand
        have hmsg : ∀ ms, typeAssertPanicMessage σF .nil tgt srcTy ms
            = typeAssertPanicMessage σ .nil tgt srcTy ms := by
          intro ms
          simpa [renameValue] using
            typeAssertPanicMessage_ren ρ hS.types_eq (v := .nil)
              (tgt := tgt) (src := srcTy) (missing := ms) (Or.inl rfl)
        simp only [renameValue]
        cases hres : resolveDefinedAliases σ tgt <;>
          (refine ExSim.bind (ExSim.refl _) ?_
           intro ms ms' hms
           subst hms
           rw [hmsg ms]
           exact ExSim.panic)
      · -- interface operand
        have hmsg : ∀ ms,
            typeAssertPanicMessage σF (.interface t (renameValue ρ i)) tgt srcTy ms
              = typeAssertPanicMessage σ (.interface t i) tgt srcTy ms := by
          intro ms
          simpa [renameValue] using
            typeAssertPanicMessage_ren ρ hS.types_eq (v := .interface t i)
              (tgt := tgt) (src := srcTy) (missing := ms) (Or.inr ⟨t, i, rfl⟩)
        simp only [renameValue]
        cases hres : resolveDefinedAliases σ tgt
        case interface iname =>
            dsimp only
            rw [firstUnsatisfiedMethod?_congr hS.types_eq hS.methods_eq
              hS.funcs_eq hS.methodSets_eq]
            refine ExSim.bind (ExSim.refl _) ?_
            intro ms ms' hms
            subst hms
            rw [hmsg ms]
            exact ExSim.panic
        all_goals (
          refine ExSim.bind (ExSim.refl _) ?_
          intro ms ms' hms
          subst hms
          rw [hmsg ms]
          exact ExSim.panic)

theorem arm_indexGet_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (b i : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .indexGet [b, i])
      (applyStrictOp σF .indexGet [renameValue ρ b, renameValue ρ i]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro n n' hn
  subst hn
  cases b <;> simp only [renameValue]
  case array vals => exact scalar_pair_SS hS (arrayGet_sim ρ vals n)
  case string gs => exact scalar_pair_SS hS (stringByteGet_sim ρ gs n)
  case slice sl =>
      refine ExSim.bind (sliceIndexLoc_sim ρ sl n) ?_
      intro l lF hl
      subst hl
      exact scalar_pair_SS hS (loadLoc_sim hS.toFrameSim l)
  -- pointer-to-array read (triage L5): load commutes, then the array
  -- projection; the nil arm's panic message is fixed text.
  case addr l =>
      refine ExSim.bind (loadLoc_sim hS.toFrameSim l) ?_
      intro bv bvF hbv
      subst hbv
      cases bv <;> simp only [renameValue]
      case array vals => exact scalar_pair_SS hS (arrayGet_sim ρ vals n)
      all_goals exact ExSim.stuck'
  case nil => exact ExSim.panic
  all_goals exact ExSim.stuck'

theorem arm_indexAddr_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (b i : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .indexAddr [b, i])
      (applyStrictOp σF .indexAddr [renameValue ρ b, renameValue ρ i]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (indexTargetLoc_sim_SS hS b i) ?_
  intro l lF hl
  subst hl
  exact ExSim.ok ⟨by simp only [renameValue], hS⟩

theorem arm_mapGet_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (hinj : ∀ {x y : Nat}, ρ x = ρ y → x = y)
    (kt vt : Ty) (b i : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.mapGet kt vt) [b, i])
      (applyStrictOp σF (.mapGet kt vt) [renameValue ρ b, renameValue ρ i]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsMap_sim_SS ρ b) ?_
  intro mp mpF hmp
  subst hmp
  refine ExSim.bind (normalizeValueForTy_sim hS.types_eq kt i) ?_
  intro k kF hk
  subst hk
  obtain ⟨mb⟩ := mp
  cases mb with
  | none =>
      simp only [Option.map_none]
      refine ExSim.bind (checkKeyHashable_sim (ρ := ρ) hS.types_eq k false false) ?_
      intro _ _ _
      exact scalar_pair_SS hS (defaultValue_sim hS.types_eq vt)
  | some bl =>
      simp only [Option.map_some]
      refine ExSim.bind (loadLoc_sim hS.toFrameSim bl) ?_
      intro w wF hw
      subst hw
      cases w <;> simp only [renameValue]
      case mapData entries =>
          refine ExSim.bind (mapEntryIndex?_sim hinj hS.types_eq kt entries k false) ?_
          intro o o' ho
          subst ho
          cases o with
          | none => exact scalar_pair_SS hS (defaultValue_sim hS.types_eq vt)
          | some idx =>
              dsimp only
              rw [renEntries_getElem?]
              cases hidx : entries[idx]? with
              | none => exact ExSim.stuck'
              | some e =>
                  obtain ⟨k1, v1⟩ := e
                  simp only [Option.map_some]
                  exact ExSim.ok ⟨rfl, hS⟩
      all_goals exact ExSim.stuck'

theorem arm_slice3_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (b lo hi : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.sliceExpr false) [b, lo, hi])
      (applyStrictOp σF (.sliceExpr false)
        [renameValue ρ b, renameValue ρ lo, renameValue ρ hi]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro n1 n1' h1
  subst h1
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro n2 n2' h2
  subst h2
  exact applySlice_sim_SS hS b n1 n2 none

theorem arm_slice4_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (b lo hi mx : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.sliceExpr true) [b, lo, hi, mx])
      (applyStrictOp σF (.sliceExpr true)
        [renameValue ρ b, renameValue ρ lo, renameValue ρ hi,
          renameValue ρ mx]) := by
  simp only [applyStrictOp]
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro n1 n1' h1
  subst h1
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro n2 n2' h2
  subst h2
  refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
  intro n3 n3' h3
  subst h3
  exact applySlice_sim_SS hS b n1 n2 (some n3)

theorem arm_length_body_SS (hS : FrameSimS ρ na₀ na fr σ σF) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.lengthOf none) [v])
      (applyStrictOp σF (.lengthOf none) [renameValue ρ v]) := by
  simp only [applyStrictOp]
  cases v <;> simp only [renameValue]
  case string gs => exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  case addr l =>
      refine ExSim.bind (loadLoc_sim hS.toFrameSim l) ?_
      intro w wF hw
      subst hw
      cases w <;> simp only [renameValue]
      case array vals =>
          rw [renArr_size_SS]
          exact ExSim.ok ⟨by simp only [renameValue], hS⟩
      all_goals exact ExSim.unsupported'
  case array vals =>
      rw [renArr_size_SS]
      exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  case slice sl =>
      obtain ⟨sb, so, sln, scp⟩ := sl
      refine ExSim.bind (validateSlice_exsim_SS ρ ⟨sb, so, sln, scp⟩) ?_
      intro _ _ _
      exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  case map mp =>
      obtain ⟨mb⟩ := mp
      cases mb with
      | none => exact ExSim.ok ⟨by simp only [renameValue], hS⟩
      | some bl =>
          simp only [Option.map_some]
          refine ExSim.bind (loadLoc_sim hS.toFrameSim bl) ?_
          intro w wF hw
          subst hw
          cases w <;> simp only [renameValue]
          case mapData es =>
              rw [renEntries_size_SS]
              exact ExSim.ok ⟨by simp only [renameValue], hS⟩
          all_goals exact ExSim.stuck'
  case chan c =>
      obtain ⟨cb⟩ := c
      cases cb with
      | none => exact ExSim.ok ⟨by simp only [renameValue], hS⟩
      | some bl =>
          simp only [Option.map_some]
          refine ExSim.bind (loadLoc_sim hS.toFrameSim bl) ?_
          intro w wF hw
          subst hw
          cases w <;> simp only [renameValue]
          case chanData buf cap closed =>
              rw [renArr_size_SS]
              exact ExSim.ok ⟨by simp only [renameValue], hS⟩
          all_goals exact ExSim.stuck'
  all_goals exact ExSim.unsupported'

theorem arm_length_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (typ : Option Ty) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.lengthOf typ) [v])
      (applyStrictOp σF (.lengthOf typ) [renameValue ρ v]) := by
  rcases typ with _ | ty
  · exact arm_length_body_SS hS v
  · cases ty
    case pointer elem =>
        cases elem
        case array n e =>
            simp only [applyStrictOp]
            exact ExSim.ok ⟨by simp only [renameValue], hS⟩
        all_goals exact arm_length_body_SS hS v
    all_goals exact arm_length_body_SS hS v

theorem arm_capacity_body_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.capacityOf none) [v])
      (applyStrictOp σF (.capacityOf none) [renameValue ρ v]) := by
  simp only [applyStrictOp]
  cases v <;> simp only [renameValue]
  case addr l =>
      refine ExSim.bind (loadLoc_sim hS.toFrameSim l) ?_
      intro w wF hw
      subst hw
      cases w <;> simp only [renameValue]
      case array vals =>
          rw [renArr_size_SS]
          exact ExSim.ok ⟨by simp only [renameValue], hS⟩
      all_goals exact ExSim.unsupported'
  case array vals =>
      rw [renArr_size_SS]
      exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  case slice sl =>
      obtain ⟨sb, so, sln, scp⟩ := sl
      refine ExSim.bind (validateSlice_exsim_SS ρ ⟨sb, so, sln, scp⟩) ?_
      intro _ _ _
      exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  case chan c =>
      obtain ⟨cb⟩ := c
      cases cb with
      | none => exact ExSim.ok ⟨by simp only [renameValue], hS⟩
      | some bl =>
          simp only [Option.map_some]
          refine ExSim.bind (loadLoc_sim hS.toFrameSim bl) ?_
          intro w wF hw
          subst hw
          cases w <;> simp only [renameValue]
          case chanData buf cap closed =>
              exact ExSim.ok ⟨by simp only [renameValue], hS⟩
          all_goals exact ExSim.stuck'
  all_goals exact ExSim.unsupported'

theorem arm_capacity_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (typ : Option Ty) (v : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.capacityOf typ) [v])
      (applyStrictOp σF (.capacityOf typ) [renameValue ρ v]) := by
  rcases typ with _ | ty
  · exact arm_capacity_body_SS hS v
  · cases ty
    case pointer elem =>
        cases elem
        case array n e =>
            simp only [applyStrictOp]
            exact ExSim.ok ⟨by simp only [renameValue], hS⟩
        all_goals exact arm_capacity_body_SS hS v
    all_goals exact arm_capacity_body_SS hS v

/-- The IEEE `min`/`max` selection commutes with the renaming: floats
are loc-free, so both sides run the SAME computation and every ok
result renames to itself (triage L3). -/
theorem floatMinMax_sim_SS (isMin : Bool) (a b : GoValue) :
    ExSim (fun r rF => rF = renameValue ρ r)
      (floatMinMax isMin a b)
      (floatMinMax isMin (renameValue ρ a) (renameValue ρ b)) := by
  cases a <;> cases b <;> simp only [renameValue] <;>
    first
    | exact ExSim.stuck'
    | skip
  case float.float x kx y ky =>
    refine exsim_ren_self_SS ρ ?_
    intro r hr
    unfold floatMinMax at hr
    (repeat' split at hr) <;>
      first
      | (simp at hr; done)
      | (simp only [pure_eq_ok, Except.ok.injEq] at hr; subst hr;
         simp [renameValue])

theorem arm_min_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) (vs : List GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .minOf (v :: vs))
      (applyStrictOp σF .minOf (renameValue ρ v :: renameValueList ρ vs)) := by
  simp only [applyStrictOp]
  have hfl : anyFloatOperand (renameValue ρ v :: renameValueList ρ vs)
      = anyFloatOperand (v :: vs) := by
    simpa [renameValueList] using anyFloatOperand_ren ρ (v :: vs)
  rw [hfl]
  by_cases hf : anyFloatOperand (v :: vs) = true
  · -- the IEEE float fold (triage L3)
    rw [if_pos hf, if_pos hf]
    refine ExSim.bind (R := fun b b' => b' = renameValue ρ b) ?_ ?_
    · rw [renameValueList_eq_map]
      refine forIn_sim (t := renameValue ρ) rfl ?_
      intro w _ x y hxy
      subst hxy
      refine ExSim.bind (floatMinMax_sim_SS (ρ := ρ) true x w) ?_
      intro c c' hc
      subst hc
      exact ExSim.ok rfl
    · intro b b' hb
      exact ExSim.ok ⟨hb, hS⟩
  · rw [if_neg hf, if_neg hf]
    refine ExSim.bind (R := fun b b' => b' = renameValue ρ b) ?_ ?_
    · rw [renameValueList_eq_map]
      refine forIn_sim rfl ?_
      intro w _ x y hxy
      subst hxy
      refine ExSim.bind (valueLess_sim_SS ρ rfl rfl) ?_
      intro c c' hc
      subst hc
      cases c
      · exact ExSim.ok rfl
      · exact ExSim.ok rfl
    · intro b b' hb
      exact ExSim.ok ⟨hb, hS⟩

theorem arm_max_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (v : GoValue) (vs : List GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .maxOf (v :: vs))
      (applyStrictOp σF .maxOf (renameValue ρ v :: renameValueList ρ vs)) := by
  simp only [applyStrictOp]
  have hfl : anyFloatOperand (renameValue ρ v :: renameValueList ρ vs)
      = anyFloatOperand (v :: vs) := by
    simpa [renameValueList] using anyFloatOperand_ren ρ (v :: vs)
  rw [hfl]
  by_cases hf : anyFloatOperand (v :: vs) = true
  · -- the IEEE float fold (triage L3)
    rw [if_pos hf, if_pos hf]
    refine ExSim.bind (R := fun b b' => b' = renameValue ρ b) ?_ ?_
    · rw [renameValueList_eq_map]
      refine forIn_sim (t := renameValue ρ) rfl ?_
      intro w _ x y hxy
      subst hxy
      refine ExSim.bind (floatMinMax_sim_SS (ρ := ρ) false x w) ?_
      intro c c' hc
      subst hc
      exact ExSim.ok rfl
    · intro b b' hb
      exact ExSim.ok ⟨hb, hS⟩
  · rw [if_neg hf, if_neg hf]
    refine ExSim.bind (R := fun b b' => b' = renameValue ρ b) ?_ ?_
    · rw [renameValueList_eq_map]
      refine forIn_sim rfl ?_
      intro w _ x y hxy
      subst hxy
      refine ExSim.bind (valueLess_sim_SS ρ rfl rfl) ?_
      intro c c' hc
      subst hc
      cases c
      · exact ExSim.ok rfl
      · exact ExSim.ok rfl
    · intro b b' hb
      exact ExSim.ok ⟨hb, hS⟩

theorem arm_runeAt_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (sv ov : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .runeAt [sv, ov])
      (applyStrictOp σF .runeAt [renameValue ρ sv, renameValue ρ ov]) := by
  simp only [applyStrictOp]
  cases sv <;> simp only [renameValue]
  case string gs =>
      refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
      intro n n' hn
      subst hn
      by_cases hneg : n < 0
      · rw [if_pos hneg]
        exact ExSim.stuck'
      · rw [if_neg hneg, if_neg hneg]
        simp only [pure_bind]
        exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  all_goals exact ExSim.stuck'

theorem arm_runeSizeAt_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (sv ov : GoValue) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ .runeSizeAt [sv, ov])
      (applyStrictOp σF .runeSizeAt [renameValue ρ sv, renameValue ρ ov]) := by
  simp only [applyStrictOp]
  cases sv <;> simp only [renameValue]
  case string gs =>
      refine ExSim.bind (valueAsInt_sim_SS ρ rfl) ?_
      intro n n' hn
      subst hn
      by_cases hneg : n < 0
      · rw [if_pos hneg]
        exact ExSim.stuck'
      · rw [if_neg hneg, if_neg hneg]
        simp only [pure_bind]
        exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  all_goals exact ExSim.stuck'

theorem arm_defaultValueOf_SS (hS : FrameSimS ρ na₀ na fr σ σF) (ty : Ty) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.defaultValueOf ty) [])
      (applyStrictOp σF (.defaultValueOf ty) []) := by
  simp only [applyStrictOp]
  exact scalar_pair_SS hS (defaultValue_sim hS.types_eq ty)

theorem arm_nilLit_SS (hS : FrameSimS ρ na₀ na fr σ σF) (typ : Option Ty) :
    ExSim (PairSim_SS ρ na₀ na fr)
      (applyStrictOp σ (.nilLit typ) [])
      (applyStrictOp σF (.nilLit typ) []) := by
  rcases typ with _ | ty
  · exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  · cases ty <;>
      first
      | exact ExSim.stuck'
      | exact ExSim.unsupported'
      | exact ExSim.ok ⟨by simp only [renameValue], hS⟩
      | (simp only [applyStrictOp]
         exact scalar_pair_SS hS (defaultValue_sim hS.types_eq _))

/-! ## The whole op table -/

set_option maxHeartbeats 1600000 in
theorem applyStrictOp_sim_SS (hS : FrameSimS ρ na₀ na fr σ σF)
    (hinj : ∀ {x y : Nat}, ρ x = ρ y → x = y)
    (op : StrictOp) (vs : List GoValue) :
    ExSim (fun (r rF : GoValue × ExecState) =>
        rF.1 = renameValue ρ r.1 ∧ FrameSimS ρ na₀ na fr r.2 rF.2)
      (applyStrictOp σ op vs)
      (applyStrictOp σF op (renameValueList ρ vs)) := by
  cases op with
  | add =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_add_SS hS l r
      · exact ExSim.stuck'
  | sub =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_sub_SS hS l r
      · exact ExSim.stuck'
  | mul =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_mul_SS hS l r
      · exact ExSim.stuck'
  | div =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_div_SS hS l r
      · exact ExSim.stuck'
  | mod =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_mod_SS hS l r
      · exact ExSim.stuck'
  | shiftLeft =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_shiftLeft_SS hS l r
      · exact ExSim.stuck'
  | shiftRight =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_shiftRight_SS hS l r
      · exact ExSim.stuck'
  | bitAnd =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_bitAnd_SS hS l r
      · exact ExSim.stuck'
  | bitOr =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_bitOr_SS hS l r
      · exact ExSim.stuck'
  | bitXor =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_bitXor_SS hS l r
      · exact ExSim.stuck'
  | bitClear =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_bitClear_SS hS l r
      · exact ExSim.stuck'
  | bitNeg =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_bitNeg_SS hS v
      · exact ExSim.stuck'
  | not =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_not_SS hS v
      · exact ExSim.stuck'
  | neg =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_neg_SS hS v
      · exact ExSim.stuck'
  | floatLit num den kind =>
      rcases vs with _ | ⟨x, t⟩
      · simp only [renameValueList]; exact arm_floatLit_SS hS num den kind
      · exact ExSim.stuck'
  | eqCmp ty =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_eq_SS hS hinj ty l r
      · exact ExSim.stuck'
  | neqCmp ty =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_neq_SS hS hinj ty l r
      · exact ExSim.stuck'
  | atMostCmp =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_atMost_SS hS l r
      · exact ExSim.stuck'
  | atLeastCmp =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_atLeast_SS hS l r
      · exact ExSim.stuck'
  | lessCmp =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_less_SS hS l r
      · exact ExSim.stuck'
  | greaterCmp =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_greater_SS hS l r
      · exact ExSim.stuck'
  | convert ty =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_convert_SS hS ty v
      · exact ExSim.stuck'
  | bytesFromString =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_bytesFromString_SS hS v
      · exact ExSim.stuck'
  | stringFromByteSlice =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_stringFromByteSlice_SS hS v
      · exact ExSim.stuck'
  | stringFromRune =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_stringFromRune_SS hS v
      · exact ExSim.stuck'
  | runesFromString =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_runesFromString_SS hS v
      · exact ExSim.stuck'
  | stringFromRuneSlice =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_stringFromRuneSlice_SS hS v
      · exact ExSim.stuck'
  | deref ty =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_deref_SS hS ty v
      · exact ExSim.stuck'
  | fieldGet tid fname =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_fieldGet_SS hS tid fname v
      · exact ExSim.stuck'
  | fieldAddr tid fname =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_fieldAddr_SS hS tid fname v
      · exact ExSim.stuck'
  | addrOfDeref =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_addrOfDeref_SS hS v
      · exact ExSim.stuck'
  | structLit ty => exact arm_structLit_SS hS ty vs
  | arrayLit n elem keys => exact arm_arrayLit_SS hS n elem keys vs
  | toInterface tgt dyn =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_toInterface_SS hS tgt dyn v
      · exact ExSim.stuck'
  | typeAssert tgt srcTy =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_typeAssert_SS hS tgt srcTy v
      · exact ExSim.stuck'
  | indexGet =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_indexGet_SS hS l r
      · exact ExSim.stuck'
  | indexAddr =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_indexAddr_SS hS l r
      · exact ExSim.stuck'
  | mapGet kt vt =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_mapGet_SS hS hinj kt vt l r
      · exact ExSim.stuck'
  | sliceExpr hasMax =>
      cases hasMax
      · rcases vs with _ | ⟨b, _ | ⟨lo, _ | ⟨hi, _ | ⟨x, t⟩⟩⟩⟩
        · exact ExSim.stuck'
        · exact ExSim.stuck'
        · exact ExSim.stuck'
        · simp only [renameValueList]; exact arm_slice3_SS hS b lo hi
        · exact ExSim.stuck'
      · rcases vs with _ | ⟨b, _ | ⟨lo, _ | ⟨hi, _ | ⟨mx, _ | ⟨x, t⟩⟩⟩⟩⟩
        · exact ExSim.stuck'
        · exact ExSim.stuck'
        · exact ExSim.stuck'
        · exact ExSim.stuck'
        · simp only [renameValueList]; exact arm_slice4_SS hS b lo hi mx
        · exact ExSim.stuck'
  | lengthOf typ =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_length_SS hS typ v
      · exact ExSim.stuck'
  | capacityOf typ =>
      rcases vs with _ | ⟨v, _ | ⟨x, t⟩⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_capacity_SS hS typ v
      · exact ExSim.stuck'
  | defaultValueOf ty =>
      rcases vs with _ | ⟨x, t⟩
      · simp only [renameValueList]; exact arm_defaultValueOf_SS hS ty
      · exact ExSim.stuck'
  | nilLit typ =>
      rcases vs with _ | ⟨x, t⟩
      · simp only [renameValueList]; exact arm_nilLit_SS hS typ
      · exact ExSim.stuck'
  | funcValOf fid => exact ExSim.ok ⟨by simp only [renameValue], hS⟩
  | minOf =>
      rcases vs with _ | ⟨v, t⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_min_SS hS v t
  | maxOf =>
      rcases vs with _ | ⟨v, t⟩
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_max_SS hS v t
  | runeAt =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_runeAt_SS hS l r
      · exact ExSim.stuck'
  | runeSizeAt =>
      rcases vs with _ | ⟨l, _ | ⟨r, _ | ⟨x, t⟩⟩⟩
      · exact ExSim.stuck'
      · exact ExSim.stuck'
      · simp only [renameValueList]; exact arm_runeSizeAt_SS hS l r
      · exact ExSim.stuck'

end


end GoLean.Frame
