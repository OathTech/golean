...
**`import GoLeanProofs.Examples.Reverse` NOW REACHES `reverse_ok`**
...
This file is `GoLeanProofs.Examples.Reverse.Core`; the module named
`GoLeanProofs.Examples.Reverse` is a thin AGGREGATOR importing this
one and the swap shard, and the swap shard now imports Core rather
than the aggregator — so the graph stays acyclic and the example's own
name is again its single entry point. Before the repair the shard
imported the root, which is why the re-export was inexpressible: it
would have closed a cycle, and Lean's import graph is acyclic. Either
`GoLeanProofs.Examples.Reverse` or `…Reverse.HarnessV` gives you
`reverse_ok`; the aggregator additionally gives you the `_v1` pair and
`reverse_framed`.
