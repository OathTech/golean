...
**`import GoLeanProofs.Examples.MinMax` NOW REACHES `minmax_ok`**
(G4.2 DAG repair, 2026-08-15 — the structural follow-up C-H4/C-H5
recorded in `docs/2026-08-15_phase2-premerge-audit.md`, DISCHARGED).
This file is `GoLeanProofs.Examples.MinMax.Core`; the module named
`GoLeanProofs.Examples.MinMax` is a thin AGGREGATOR importing this one
and the swap shard, and the swap shard now imports Core rather than
the aggregator — so the graph stays acyclic and the example's own name
is again its single entry point. Before the repair the shard imported
the root, which is why the re-export was inexpressible: it would have
closed a cycle, and Lean's import graph is acyclic. Either
`GoLeanProofs.Examples.MinMax` or `…MinMax.HarnessR` gives you
`minmax_ok`; the aggregator additionally gives you the `_v1` pair and
the `minmax_framed` pair.
