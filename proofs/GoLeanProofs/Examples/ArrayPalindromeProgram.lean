...
/-!
# The `palin` example — the frontend's lowering, pinned
(gallery campaign G1, guardrails wave, 2026-08-15)

`palinLowered` is the native frontend's ACTUAL lowering of the
canonical corpus source (`Corpus/coverage/exec/examples/palin/main.go`,
differentially green against `go run`), pinned by `scripts/check-golden`
against `baselines/golden/palin-lowered.repr`. THIS DEF IS GENERATED from
...
/-- The frontend's lowering of the `palin` subject, verbatim
(`scripts/check-golden` pins it). -/
def palinLowered : Program :=
  { typeDefs := #[({ key := "struct{}" }, GoLean.GoCore.TypeDef.struct #[])],
    funcs := #[{ id := { key := "isPalindrome" },
                 args := #[{ id := "s",
