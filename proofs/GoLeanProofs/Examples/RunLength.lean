...
/-!
# RunLength — the `rle` example (Gallery Campaign G1, guardrails wave)
...
`RunLengthProgram`, itself pinned by `scripts/check-golden` against
`baselines/golden/rle-lowered.repr`) and the named harness
...
aggregator's `import GoLeanProofs.Examples.RunLength` reaches it by name
...
hand-written, so `rleHarnessRFunc_pin` checks a transcription that is
