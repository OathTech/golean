...
/-!
# The reverse example — the frontend's lowering, pinned (verified-examples
arc slice 2b, 2026-08-13)

`reverseLowered` is the native frontend's ACTUAL lowering of the
canonical corpus source (`Corpus/coverage/exec/examples/reverse/main.go`,
5/5 differentially green against `go run`, including odd/even lengths,
singleton, empty, and an int64-boundary value), pinned by
`scripts/check-golden` against `baselines/golden/reverse-lowered.repr`.
...
/-- The frontend's lowering of the reverse subject, verbatim
(`scripts/check-golden` pins it). -/
def reverseLowered : Program :=
  { typeDefs := #[({ key := "struct{}" }, GoLean.GoCore.TypeDef.struct #[])],
    funcs := #[{ id := { key := "reverse" },
                 args := #[{ id := "s",
                             ...
                             GoLean.GoCore.Stmt.seqn
                               #[GoLean.GoCore.Stmt.initialization
                                 ...
                                 GoLean.GoCore.Stmt.assign
                                   ...
                                   (GoLean.GoCore.Expr.var "$c0")],
                             GoLean.GoCore.Stmt.call #[] { key := "reverse" } #[GoLean.GoCore.Expr.var "s"],
                             GoLean.GoCore.Stmt.seqn
                               #[GoLean.GoCore.Stmt.assign
