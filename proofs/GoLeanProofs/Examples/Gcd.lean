...
/-- **THE HEADLINE — framed total correctness, full domain, exact
value**: for every `a, b` in the full uint64 × uint64 domain, every
disjoint frame, and every admissible allocator bound, execution of
`$callres = gcd(a, b)` from the framed seed completes normally — past
one fuel bound, at every nondeterminism-choice stream — with EXACTLY
`Nat.gcd a b` in the result cell, and every frame cell preserved
verbatim.
...
framed driver, at any fuel and any choice stream, delivers exactly
`Nat.gcd a b` and preserves the frame — derived from the headline via
...
theorem gcd_readout (a b : Nat) (ha : a < 2 ^ 64) (hb : b < 2 ^ 64)
  ...
  normal_readout_of_total (gcd_ok a b ha hb fr na hfr hwf)
