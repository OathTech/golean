...
/-!
# The `dedup` example — the frontend's lowering, pinned
(gallery campaign G1, guardrails wave, 2026-08-15)

`dedupLowered` is the native frontend's ACTUAL lowering of the
canonical corpus source (`Corpus/coverage/exec/examples/dedup/main.go`,
differentially green against `go run`), pinned by `scripts/check-golden`
against `baselines/golden/dedup-lowered.repr`. THIS DEF IS GENERATED from
...
/-- The frontend's lowering of the `dedup` subject, verbatim
(`scripts/check-golden` pins it). -/
def dedupLowered : Program :=
  { typeDefs := #[({ key := "struct{}" }, GoLean.GoCore.TypeDef.struct #[])],
    funcs := #[{ id := { key := "dedupAdjacent" },
                 args := #[{ id := "s",
                 ...
                 body := GoLean.GoCore.Stmt.block
                           ...
                           #[GoLean.GoCore.Stmt.seqn
                               #[GoLean.GoCore.Stmt.initialization
                                   { id := "k", typ := GoLean.GoCore.Ty.int (GoLean.GoCore.IntKind.int) },
