...
body): within 24 steps the state advances to the erased snapshot with
`best` at `max bv p.2` and one fresh dead value cell — the per-
...
theorem wcRangeIter_generic
    ...
    (hStB : ∀ (kvs rem : List (Int × Nat)) (tail : Heap) (B na₀ na : Nat)
      ...
      stepFn (S kvs ivP false tail na)
        ...
        = .ok (.next (.storeK [] [] (.seqn #[]) (env4R envRBg B na₀)
            ...
            ch))
    (kvs rem : List (Int × Nat)) (idx : Nat) (p : Int × Nat)
    ...
    ∃ (k : Nat) (tail' : Heap),
      ...
      ∧ stepFnIter k (S kvs ivP false tail na)
        ...
        = .ok (rangeHeadR envRBg kRg B (rem.eraseIdx idx),
...
invariant `max best (maxOf remaining) = const` (GAP-R1 closure,
2026-08-15: the §10b induction itself now lives in the kit). The
statement is unchanged from the pre-closure layer. -/
theorem wcRange_generic
  ...
  intro m kvs rem hm bv B na tail ch hrem hbv hB hBna hbest htail
  obtain ⟨k, d', ch', hk, hP', hrun⟩ :=
    mapPickLoop_generic
      (T := fun d : Heap × Nat × Nat => S kvs ivP false d.1 d.2.1)
      (cfg := fun r => rangeHeadR envRBg kRg B r)
      (exitCfg := .next (kRg B))
      (P := fun d r =>
        d.2.2 ≤ bound ∧ (∀ q ∈ r, q.2 ≤ bound) ∧ na ≤ d.2.1 ∧ B < d.2.1
        ∧ Heap.lookup d.1 (.base ⟨B⟩) = some (u64cell ((d.2.2 : Nat) : Int))
        ∧ DeadFrom d.1 d.2.1
      ...
      (fun d r idx p ch₀ ch₂ hcons hidx hp hP => by
        ...
        obtain ⟨k₁, tl', hk₁, hb', htl', hrun₁⟩ :=
          wcRangeIter_generic envRBg kRg S ivP base0 hEnvBest hPick hR4b
            hVarC hVarBest hStB kvs r idx p ch₀ ch₂ B d.2.1 d.2.2 d.1
            hcons hidx hp hvnorm hB hBna' hbest' htl
      ...
      (fun d ch₀ _ =>
        segRexit_g envRBg kRg (S kvs ivP false d.1 d.2.1) B ch₀)
