...
/-!
# Kadane — the `kadane` example (Gallery Campaign G1, guardrails wave)
...
`KadaneProgram`, itself pinned by `scripts/check-golden` against
`baselines/golden/kadane-lowered.repr`) and the named harness
...
Go source: `Corpus/coverage/exec/examples/kadane/main.go`,
...
hand-written, so `kadaneHarnessRFunc_pin` checks a transcription that is
