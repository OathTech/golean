...
/-!
# The map word-count example — the frontend's lowering, pinned
(verified-examples arc slice 2c, 2026-08-13)

`wordCountLowered` is the native frontend's ACTUAL lowering of the
canonical corpus source (`Corpus/coverage/exec/examples/wordcount/main.go`,
differentially green against `go run`), pinned by
`scripts/check-golden` against `baselines/golden/wordcount-lowered.repr`.
...
/-- The frontend's lowering of the wordcount subject, verbatim
(`scripts/check-golden` pins it). -/
def wordCountLowered : Program :=
  { typeDefs := #[({ key := "struct{}" }, GoLean.GoCore.TypeDef.struct #[])],
    funcs := #[{ id := { key := "maxCount" },
                 args := #[{ id := "words",
