...
/-!
# The `twosum` example — the frontend's lowering, pinned
(gallery campaign G1, guardrails wave, 2026-08-15)

`twosumLowered` is the native frontend's ACTUAL lowering of the
canonical corpus source (`Corpus/coverage/exec/examples/twosum/main.go`,
differentially green against `go run`), pinned by `scripts/check-golden`
against `baselines/golden/twosum-lowered.repr`. THIS DEF IS GENERATED from
...
/-- The frontend's lowering of the `twosum` subject, verbatim
(`scripts/check-golden` pins it). -/
def twosumLowered : Program :=
                 ...
                 body := GoLean.GoCore.Stmt.block
                           ...
                           #[GoLean.GoCore.Stmt.seqn
                               #[GoLean.GoCore.Stmt.initialization
                                   { id := "n", typ := GoLean.GoCore.Ty.int (GoLean.GoCore.IntKind.uint64) },
