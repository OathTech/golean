...
/-!
# SelectionSort — the `selsort` example (Gallery Campaign G1, guardrails wave)
...
`SelectionSortProgram`, itself pinned by `scripts/check-golden` against
`baselines/golden/selsort-lowered.repr`) and the named harness
...
aggregator's `import GoLeanProofs.Examples.SelectionSort` reaches it by name
...
hand-written, so `selsortHarnessRFunc_pin` checks a transcription that is
