...
/-!
# DedupAdjacent — the `dedup` example (Gallery Campaign G1, guardrails wave)
...
`DedupAdjacentProgram`, itself pinned by `scripts/check-golden` against
`baselines/golden/dedup-lowered.repr`) and the named harness
...
aggregator's `import GoLeanProofs.Examples.DedupAdjacent` reaches it by name
(the C-H4/C-H5 shape, adopted from birth).

Go source: `Corpus/coverage/exec/examples/dedup/main.go`,
...
hand-written, so `dedupHarnessRFunc_pin` checks a transcription that is
