...
/-!
# FibMemo — the `fibmemo` example (Gallery Campaign G1, guardrails wave)
...
`FibMemoProgram`, itself pinned by `scripts/check-golden` against
`baselines/golden/fibmemo-lowered.repr`) and the named harness
...
Go source: `Corpus/coverage/exec/examples/fibmemo/main.go`,
