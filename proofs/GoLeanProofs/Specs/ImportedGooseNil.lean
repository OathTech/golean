...
same staleness caveat as the batch-1 pilot (`ImportedGooseBlock.lean`):
NOT wired into `scripts/check-golden` pending the parked P1 decision
(docs/goose-parity-parked.md); a frontend change can silently date this
term — recorded, not hidden.
