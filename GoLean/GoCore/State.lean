...
* `tryLock`      — TryLock/TryRLock's spurious-failure member
                   ...
                   scheduling pick.
* `unseqPanic`   — the spec-UNSEQUENCED sibling panic order (latitude E13
                   option (b), RULED [USER] 2026-09-05, relayed — design
                   `docs/2026-09-05_e13-b-design.md`): at a
                   `.panicking _ (.probeK _)` configuration — an
                   `Stmt.unseqProbe` operand PANICKED at its lexical
                   position ahead of a sibling ordered event — the pick
                   is DEFER (slot 0: the panic is not raised here; the
                   operand's residual re-evaluation after the sibling
                   events raises what it raises then) or RAISE (slot 1:
                   the panic propagates now). Bound exactly 2 there;
                   NOWHERE else does the probe consult (a probe whose
                   operand evaluates to a value pops nothing — the consult
                   is on the panic path only, so an ordinary `a[i] + f()`
                   row's stream is untouched unless `a[i]` panics before
                   `f`). The envelope statement lives at `Stmt.unseqProbe`
                   (Syntax.lean); consumed in `stepFn`'s `.panicking`
                   arm (StepFn.lean); projection arm in `seqConsumption`
                   (Machine.lean). A DATA pick, not a scheduling pick.
* `repanicCollapse` — the abort line's `[recovered, repanicked]` marker
                   when a RECOVERED panic value is re-panicked with an
                   EQUAL payload (BUG-004 item 1; landing chunk L3,
                   `docs/2026-09-07_land-panic-text-tape.md`, under the
                   BUG-087 panic-text ruling «demonic choice so both are
                   admitted», [USER] 2026-09-03 relayed). gc decides the
                   marker by eface IDENTITY (`preprintpanics`,
                   runtime/panic.go:715 at the pin — the type word AND
                   the data pointer): `panic(r)` passes the recovered box
                   through and COLLAPSES the two lines into one
                   `… [recovered, repanicked]`; a re-boxed equal value
                   (`panic(r.(string))`, a runtime-computed string) gets
                   a fresh allocation and prints the two-line
                   `… [recovered] ⏎ ⇥panic: …`; two literal constants
                   collapse or not by linker dedup; every go ≤ 1.24 prints
                   the two-line form for all of them (the collapse is CL
                   645916, go1.25). The machine has no boxing identity, so
                   relative to its value-level state the marker is
                   latitude: slot 0 = COLLAPSE (` [recovered,
                   repanicked]`, the member the corpus pinned), slot 1 =
                   the two-line form's first line (` [recovered]`).
                   Width 2 exactly when the abort's head entry is
                   recovered and its successor's payload is EQUAL
                   (`repanicCollapseWidth`, Machine.lean — the envelope
                   statement is `renderPanicHead`'s docstring); unequal
                   adjacent payloads cannot share a box and stay the
                   forced ` [recovered]` (bound 1, no pop). Consumed at
                   THE ABORT and nowhere else — `abortConsult` (Machine.
                   lean), drawn by `stepFn`'s `.panicking _ .stop` arm
                   (StepFn.lean) and the pool's tombstone arm
                   (`stepThread`, Multi.lean — recorded in the abort
                   `StepEvent`); projection arms in `seqConsumption` and
                   `poolConsumption`. A DATA pick, not a scheduling pick.
* `unseqNext`    — the `unseq` scheduler's pick (the evaluation-order
                   model v2.1, `docs/2026-09-16_evaluation-order-model-v2.md`
                   §3.3/§3.5; Stage B, lane `core/unseq-scheduler-b-0916`,
                   2026-09-16; the mechanism RULED [USER] Mike 2026-09-16,
                   relayed by the [AGENT] coordinator — «I think the Cerberus
                   model is the correct one»): at a sweep frame's PICK
                   position `.next (.unseqK … .pick _)` with ≥ 2 READY
                   occurrences, WHICH ready occurrence runs next — slot j =
                   the j-th ready occurrence in canonical rank order
                   (`UnseqGraph.ready`, Unseq.lean: THE one ready
                   computation execution, the `Step.unseqPick` premise and
                   `seqConsumption` share); bound = the number of ready
                   occurrences EXACTLY; a singleton ready set is a bound-1
                   consult and pops nothing (G-U). Consumed in
                   `stepUnseqNext` (StepFn.lean); projection arm in
...
def ChoiceSite.canonicalSlot0 : ChoiceSite → String
  ...
  | .unseqPanic =>
      "DEFER (slot 0 = the unsequenced operand's early panic is NOT raised at its lexical position; the operand is re-evaluated at its residual position after the sibling ordered events and raises whatever it raises there — the pre-E13-re-envelope machine's only member, so the empty/default stream reproduces every pre-change trajectory byte for byte; gc's realization for index/dereference/division/shift/slice-to-array-conversion operands); slot 1 = RAISE (the panic propagates now, ahead of the sibling events — gc's realization for type assertions, slice expressions and interface comparisons); bound is 2 exactly at a `.panicking _ (.probeK _)` configuration and the probe consults nowhere else (a value-yielding probe pops nothing)"
  | .nilValueMethodText =>
