...
- Goose-parity buildout: charter BLESSED (user sign-off 2026-08-07;
...
- Observation-channel width/type fidelity (grossmith hunt F15, 2026-08-09): the observation JSON collapses every integer kind to {"tag":"int"} and drops defined-type identity except structs/interfaces — kind-level divergences landing on the right value are invisible to observation-eq. Fix = symmetric encoder extension (CLI.lean goValueJson + coverageharness), both sides in one change; also the first step toward symmetric document emission. Scheduling: user decision (candidate: early spec-parity slice).
