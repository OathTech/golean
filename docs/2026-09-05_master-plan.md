...
|---|---|---|---|
| (1) | **Consumer-contract spike (Gate A)**: corrected bridge statements PROVED on a two-choice example (both directions); the admissible context class and its laws (the recover counterexample as the first test); typed entry/admission predicates (`WireWellFormed`, `ProgramWellTyped`, valid entry/arguments, support contract); a minimal iris-lean `Language` adapter — IN-REPO per Q1, but OUTSIDE this package's default dependency graph and NOT built by `scripts/ci` (a separate Lake target or package; iris-lean is not a dependency of the shipped semantics). Sized after, not before. | **A1 LANDED at `8db2d6da`; bounded A2 LANDED at `700128f3` after independent review [USER-authorized, AGENT-executed] 2026-09-05** — [contract and scope](2026-09-05_gate-a1-contract.md), [review and proposed next tasks](2026-09-05_gate-a1-review-and-next-phase.md). [A2's implementation](2026-09-05_iris-customer-design.md) exercises initialized Iris ownership and actual entry-to-readout through the singleton pool driver; [independent review: PASS](2026-09-05_iris-customer-review.md). General composition, typed admission and general labeled/concurrent adequacy remain open; the whole of Gate A is not complete. | NEW; supersedes §3.A item 8's "I5 last, 2 sessions" and §5.3's timing — I5 becomes the OUTPUT of Gate A, not a late routine item |
