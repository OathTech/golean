...
FIXTURE = SCRATCH / "promoted-program-private"
for tree in ["GoLean", "Tests"]:
    shutil.copytree(ROOT / ".lake/build/lib/lean" / tree, FIXTURE / tree)
source = FIXTURE / "GoLean/GoCore/ProgramTrace.lean"
source.write_text((ROOT / "GoLean/GoCore/ProgramTrace.lean").read_text() + "\nprivate axiom reviewedPromotedProgramHole : False\n")
...
for label, cwd, harness in [
    ...
    assert result.returncode == 1 and "forbidden axiom" in result.stdout and "reviewedPromotedProgramHole" in result.stdout, label
print("Promoted ProgramTrace unused private axiom: compiled successfully and rejected by all three post-import audits.")
