...
|---|---|---|---|---|
| O1 | An executable semantics at all + `go run` differential validation (**1483** exec + 311 negative cases, six-lane taxonomy, tracked baseline, zero-drift gates; exec figure corrected at the arc-end delta review — "1194" was the count at the 2026-08-07 seeding and had gone stale in the self-UNFAVORABLE direction while citing the live baseline; commands emitting the figures: `awk -F'\t' '$1!~/^#/{n++} END{print n}' baselines/native-full.tsv` → 1483, `scripts/coverage-negative-manifest --list \| wc -l` → 311) | design note D9; `baselines/native-full.tsv` | Their GooseLang interpreter + semantics corpus was DELETED with the old semantics (perennial `eb748d43e`, 2026-01-30); the new semantics has NO interpreter and no differential harness (research note §5; `cmd/test_gen -coq` still references the absent `Perennial.goose_lang.interpreter`) | **DEL** on their part (casualty of the rewrite); our differentiator |
...
| O7 | The sequential-conservation theorem (`execProg_single_eq_execStmt`): every sequential result in the transferable classes is the pool result verbatim — 33 sequential designated statements transfer unrestated (arithmetic at the D3 designation, 2026-08-10: 48 designated − 15 concurrent-carrier — the four spec-parity additions are all pool-carrier, so the sequential count is unchanged; was 44 − 11); corpus bit-identity as the empirical twin | slice-2 log, `MultiSound.lean` | No analogue needed (no sequential machine) — but nothing plays this role of a machine-level growth-by-extension guarantee | ours-specific asset |
...
the D3 curation says so. (THE CURATION HAPPENED — user ruling
2026-08-10: `compareNilToNilSpecC` + `compareNilToNilReadoutC` (§7.1
exemplar row) and the fuel-free `dspCert` + `dspAllSchedules` (§7.2
dsp row) are DESIGNATED — Challenge/Solution + judge config +
statement-TCB list, 44 → 48; the fuel-based kernel-evidence siblings
stay undesignated per the ruling. Every other row remains
undesignated.)
...
| semantics/new `testNilDefault` | `wp_testNilDefault`, new.v:9, **Qed** :13 | `nilDefaultSpecC` (`Specs/GooseParityNewWP.lean`, slice-6 tranche) | `nilDefaultReadoutC`; + `nilDefaultTotalReadout` | genuine parity row; shared delta only |
...
| actris-example (dsp) | `wp_DSPExample`, channel_dsp.v:35, **Qed** :57 (dependent-separation-protocol session) | `ChannelActris.dsp*`, verdict 42 | flagship row; `dspCert`+`dspAllSchedules` DESIGNATED (D3 user ruling 2026-08-10, fuel-free forms; `dspCert400` stays undesignated kernel evidence) |
