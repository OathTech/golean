...
_B4 status (2026-09-05, C-arc step 2, lane `c-arc-b4`): LANDED `40fd1903` + `165822ef` — `Signal`, `Config.signal`, the table `signalStep`/`signalRefusal`, rules `signalStmt`/`signal` (32 → 2; the four frame-exit `ret` twins stay as rules, the executable shares `stepFrameExit`); `.panicked` deleted (the abort is `.panicking chain .stop`, `Config.abort?`, rendered by the drivers/pool via `abortMsg`); ONE terminal; `ExecOutcome` deleted; `Thread | running c boundary | aborted msg` with the `Status`/`Done` VIEW (`Thread.status`: a definition + its agreement theorems `threadDone_status`/`threadRunnable_status`/`Thread.status_parked_iff`/`_done_normal_iff`, no in-repo consumer yet — the consumer is iris-lean) — NOT the stored `Array Status` with `parked (p : Park)`: the consumer's `Expr` is `Config`, so parked shapes stay configurations and `Park` as a type is OWED (design note §2/§7)._
