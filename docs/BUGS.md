...
## BUG-092 — a stream-consuming apply whose delivery then raises a RECOVERABLE panic returns the PRE-apply stream, UNDOING its pop: the machine's "a delivered panic consumed nothing" convention disagrees with every consumption account of it (the projection, the tracer's accountant, the enumerator's widths) at a spilling `appendSlice` whose target store panics and at a TRY head whose result delivery panics [design class; stream ACCOUNTING, no observation moves; surfaced by wave (iii)'s consumption theorem]

- Status: open
- Pinned-by: none (invisible to the differential: no readout or panic text moves, only which stream slot LATER consumptions read after such a panic — a choice-tape alignment fact; the tracer's sentinel discipline would ALARM on a reaching row and the theorem `stepFn_consumption_some`'s second disjunct states the behaviour exactly; no corpus row reaches either site — the whole-corpus per-consumption trace at `cd2a3474`/B8 has 0 sentinel alarms)
- Discovered: 2026-09-04, design-hygiene wave (iii) B8 (lane `hygiene-wave3`), while stating `stepFn_consumption_some` (MachineSound): the `some (site, b)` half could not be closed with "the step's stream is the site's pop" because `deliverS`'s panic branch returns the pre-apply stream at every apply, including the three that consult the stream INSIDE the apply (`applyStmtOp`'s spill, `applySelect`'s L2 pick, `applySyncOp`'s TRY pick). Design note `docs/2026-09-04_hygiene-wave3-design.md` §B8 "THE FINDING".

WHAT: `stepFn`'s apply arms deliver a helper's `.panic` as `.panicking … k` over the PRE-apply state AND pre-apply stream (`deliverS`, StepFn.lean — the convention every one of the former 41 conversion sites had; B2 preserved it site for site). For a stream-free apply that is exactly right (nothing was consumed). For an apply that POPS before its delivery panics, the pop is undone: the next consumption re-reads the slot the abandoned apply drew. Reachable shapes: (1) `a[i] = append(b, 1, 2, 3, 4, 5)` with `b` nil (cap 0 < 5 — a spill, `appendSpill` pops) and `i` out of range — Go evaluates the append, then panics at the assignment; the machine pops, then the phase-2 header store panics and the step returns the un-popped stream; (2) a TRY head whose `tryDeliver`/`enterRecvTargets` result store panics after the `tryLock` pop. `applySelect`'s post-pop panic path (`.inr`) already keeps the popped stream (the audit's defensive arm), so the select site is consistent. Every consumption ACCOUNT — `seqConsumption`/`poolConsumption` (B8), the retired `CLI.stepNeeds`/tracer mirror before it, the enumerator's branch widths — predicts the pop.

WHY NOT FIXED HERE: the fix is one line per streaming apply (`deliverS`'s panic branch takes the apply's POPPED stream, or equivalently the panic is delivered with the pop kept), but it changes the stream REALIZATION on those paths — which streams realize which later behaviours — an accounting decision of the kind invariant 1 of the arc plan sends to the [USER] (the arc's "semantics-preserving on the MACHINE" bar includes the tape). Wave (iii) states the machine as it is (the theorem's second disjunct) and refers the ruling; a fix lands with a tracer-lane pin (a reaching row + the sentinel check green) and a one-word amendment to `stepFn_consumption_some`.
