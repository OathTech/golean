...
## BUG-085 — a VALUE-receiver method reached through gc's autogenerated pointer wrapper on a nil `*T` panics with gc's `panicwrap` text (`value method main.T.M called using nil *T pointer`, a `runtime.Error` WITHOUT the "runtime error: " prefix); the machine raises the generic nil-dereference text

- Status: open
- Pinned-by: differential
- Cases: noodler/ifaces/mv-iface-nil-call, noodler/ifaces/iface-param-value-nil, noodler/ifaces/global-iface-value-nil
- Discovered: 2026-09-03 (the noodler lane — `docs/2026-09-03_noodler-report.md`
  finding F1; probe record `docs/evidence/2026-09-03_noodler/` —
  `probes/gc-wrapper-text/main.go`, transcript
  `transcripts/gc-wrapper-text.txt`, identical at default flags and
  `-gcflags='-N -l'`)

Shape: `type Inner struct{ v int }; func (i Inner) Val() int`, `var p *Inner`
(nil), `var v Valuer = p`, then a call of `Val` that gc routes through
the compiler-generated `(*Inner).Val` wrapper. The wrapper's nil check
calls `runtime.panicwrap` (`deps/go/src/runtime/error.go:324-348`), which
panics with `plainError("value method main.Inner.Val called using nil
*Inner pointer")` — a `runtime.Error` (`plainError.RuntimeError()`,
error.go:118-122) whose text carries NO "runtime error: " prefix. The
machine's dispatch dereferences the nil receiver and raises
`runtime error: invalid memory address or nil pointer dereference`.

| shape (gc probe, go1.26.5) | gc | machine |
| --- | --- | --- |
| `f := v.Val; f()` — method value from the interface | `value method main.Inner.Val called using nil *Inner pointer` | `runtime error: invalid memory address or nil pointer dereference` |
| `call(v)` with `//go:noinline func call(v Valuer) int { return v.Val() }` | panicwrap text | nil-deref text |
| `global = p; global.Val()` — interface in a package variable | panicwrap text | nil-deref text |
| `f := (*Inner).Val; f(p)` — method expression | panicwrap text | REFUSED (FR-3, the deref adapter) |
| `v.Val()` with `v`'s dynamic type statically known in the same function | nil-deref text (gc devirtualizes, even at `-N -l`) | nil-deref text — PASS (`noodler/ifaces/iface-dispatch-value-nil`) |
| `p.Val()` / `f := p.Val` directly on the nil pointer | nil-deref text | nil-deref text — PASS (existing `methods/nil-pointer-method-value` class) |

Spec: the panic itself is FORCED — spec#Method_values / spec#Calls make
`x.M` on a pointer to a value-receiver method shorthand for `(*x).M`,
and spec#Address_operators: "If x is nil, an attempt to evaluate *x will
cause a run-time panic" (spec#Run_time_panics: a `runtime.Error`). The
TEXT is not spec text — it is gc's realized string, the class the
latitude inventory pins to gc at R9 ("Run-time panic VALUES and message
texts — PINNED to gc's realized strings"). At this point gc is NOT
single-valued across shapes the spec treats identically: whether the
wrapper (panicwrap text) or a direct dereference (SIGSEGV text) fires is
decided by the compiler's devirtualization of the call site, a
compiler-internal choice (the R15 pattern: one implementation, two
realizations). The machine realizes exactly ONE of gc's two texts, on
every shape.

Classification: wrong-answer (panic text) at an R9-pinned point —
observed ∉ modeled on the wrapper-path shapes. Both values are
`runtime.Error`s, so the recoverable KIND agrees (`noodler/ifaces/
iface-dispatch-value-nil-recovered` PASS on the devirtualized shape);
only the message channel diverges. Fix shape ([AGENT] sketch, not
built — the noodler writes tests and records only): the interface
dispatch / interface method value arms, when the dynamic value is a nil
pointer and the selected method has a VALUE receiver, raise the
panicwrap text as a `runtime.Error` payload without the prefix (a new
payload rendering: plainError has no prefix where `errorString` does);
the devirtualized `direct` shape then needs its own record — a
deterministic gc realization the machine cannot re-derive (the
devirtualization decision), so either an explicit R9 sub-row admitting
both texts or a strict pin of the wrapper text with the `direct` row
re-classified as gc-realization latitude. The method-expression column
is FR-3's row, not this bug's.

## BUG-086 — `strconv.FormatInt` used WITHOUT `strconv.FormatUint` in the same program: the shim injection plants FormatInt's body without its FormatUint dependency, and the export dies in the type-checker (`type-check: golean-stdlib-shims.go:N: undefined: goleanShimStrconvFormatUint`) — a whole-program spurious refusal of an ALLOWLISTED stdlib function [frontend; the D-002 injection surface]
...
- Discovered: 2026-09-03 (the noodler lane — `docs/2026-09-03_noodler-report.md`
  ...
  formatint-bisect/` + `transcripts/formatint-bisect.txt`)

Mechanism (`tools/nativefrontend/stdlibshim.go`): the allowlist admits
`strconv.FormatInt` (:165-167) and the injection plants the shim
declarations named by `stdlibShimDeclNames[strconvFormatIntShimName]`,
which is `{strconvFormatIntShimName}` alone (:234). The FormatInt shim
BODY calls `goleanShimStrconvFormatUint` on both of its paths (:982,
:984). When the program does not ALSO call `strconv.FormatUint`
(whose own entry, :233, is what plants that declaration), the injected
file does not type-check and the frontend refuses the WHOLE export:
`nativefrontend: type-check: golean-stdlib-shims.go:11:16: undefined:
goleanShimStrconvFormatUint`. Every FormatInt call shape is affected
(bisect: positive/negative values, bases 2/10/16/36, constant and
variable arguments — five one-function programs, all refused; the
control program calling both functions exports cleanly). The existing
conformance pin `strconv/format-parse` calls FormatUint AND FormatInt
in one body, which is why the corpus never saw it.

Spec: no spec question — `strconv` is stdlib surface
(spec#Qualified_identifiers is the construct; the behavior is the
package's). Classification: spurious-refusal, frontend, whole-package
kill, on a function INSIDE the frozen allowlist (discrepancy D-002
freezes the injection SURFACE; this is a defect in an existing entry's
dependency set, not a widening). Fix shape ([AGENT] sketch, not built
here): add `strconvFormatUintShimName` to FormatInt's decl set at :234
— one line, no shim body changes; whether that counts as a "shim that
changes" under D-002's Fields-standard rule is the [USER]'s call at
the fix, recorded here so the fixer does not have to re-derive it.
