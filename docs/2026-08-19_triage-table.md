...
| F10 | sync methods through interface dispatch (`sync.Locker`, user interfaces) | stub at `emit.go:4679-4681`; quarantined by `NativeToIR.lean`'s `decodeFunc` — `"sync-primitive method sync.X.Y through interface dispatch (declaration-only stub: satisfaction answers; only direct statement/defer-position calls are modeled)"` | n/a (sync surface; design note §12) | `sync/iface-dispatch/{locker-box-dispatch,mutex-user-iface,wg-user-iface}` (3) | **(b)**; note the pin is itself an audit fix — before the stub pass this shape ESCAPED to runtime `stuck` — LANDED 2026-09-01 (Q-SYNCVAL slice, P-S2-6 bodied stubs; the three pins flipped green, `qrow-rulings` row 6) |
| F11 | sync methods as method values / go-statement callees | `emit.go:4739` — `unsup("sync.%s.%s as a method value (only direct statement/defer-position sync ops are modeled)")` | `spec#Method_values` for the shape | `sync/escapes/{go-stmt,method-value}` (2) | **(b)** same family (audit fix round F4; previously a runtime `stuck`) — LANDED 2026-09-01 (Q-SYNCVAL slice; method values/go callees of the modeled ops lower through the bodied stubs, both pins green) |
| F12 | composite-literal construction of a sync primitive (`&sync.Mutex{}`) | `emit.go:5222` — `unsup("composite-literal construction of sync.%s (out of scope: `var` declarations and new() are the modeled construction surface)")` | `spec#Composite_literals` for the form | `sync/composite-literal/{mutex-addr-lit,waitgroup-value-lit}` (2) | **(b)** — see the JUDGMENT in §3 for why this near-(a) row is NOT taken — LANDED 2026-09-01 (Q-SYNCLIT, `qrow-rulings` row 7: EMPTY literal ≡ zero value, both pins green; non-empty literals stay refused) |
...
| *(note beside C2, 2026-09-03 — not a (c) row)* BUG-087 panic text on a nil `*T` value-method dispatch | 0 | the SAME argument shape as C2 — `go run` and `go run -gcflags=-l` disagree on one source (devirtualized nil-deref text vs the `(*T).M` wrapper's `panicwrap` text) — but resolved the OTHER way: not a pinned red, an ENVELOPE. Both realizations are conforming `runtime.Error` texts at a spec-open point (R9), so the machine admits both as a width-2 demonic choice (`ChoiceSite.nilValueMethodText`; latitude inventory R9a; [USER] 2026-09-03 «demonic choice so both are admitted», relayed — `docs/2026-08-31_qrow-rulings.md`) and the five rows sit in the membership lane. C2 stays a red because ITS observable (init order) has no in-machine choice point yet; BUG-087's had one arm to widen |
