...
| M | `sortSlice` machine op (`slices.Sort` at int kinds) | **DONE 2026-09-04 (lane `fr4-rowm`): retired onto source-through `slices.Sort`** — `pdqsortOrdered` stenciled per element type at ANY ordered kind (strings, floats NaN-first, named kinds, inside `init()`); the FRONTEND half only: `emit.go emitSortStmt` and the ExprStmt intercept deleted, `frontendInterceptedLibraryMembers` EMPTY (intercept class 1 → 0), the reach walk marks the real member reached. The MACHINE half (`Syntax.lean` `sortSlice`, `Machine.lean`, `Ops.lean`, eqb/Race/NPDRF mentions, and the `sort-slice` decoder arm in `NativeToIR.lean`) is now UNREFERENCED by the frontend and its deletion is OWED to the hygiene arc — not touched on this lane (no `GoLean/GoCore` change; the decoder key stays, no wire-schema change). Is the op reachable other than through the retired intercept? No: `sort-slice` was emitted by `emitSortStmt` alone (grep at the retirement), so the op is dead code from the wire's point of view. | `slices/*` rows green through the real generic (11 files, byte-identical observations); `slices-sort-non-integer-refusal` FLIPPED; `stdlib-source/sort-op-shapes/{defer,go}-sort` FLIPPED; `slices/slices-sort-kinds/*` born (8); the raft twin pin MOVED (10 `slices.Sort` sites in quorum/tracker/raft/progress: `sort-slice` nodes → `slices.Sort[…]` static calls + the pdqsort closure); R13 stands as re-written at row 13 (the machine realizes gc's member — now for `Sort` too) |
