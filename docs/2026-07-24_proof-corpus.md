...
| `quorum/committed-index-real/three-all` (same pin) | `GoLean.Surface.quorumThreeAllFuncSpec` / `quorumThreeAllMeetsSpec` | **THE 3-VOTER RUNG, PAID (2026-08-01, proof-automation arc phase 3).** etcd's own `majority_commit.txt` row — `committedThreeAll()` over `MajorityConfig{1,2,3}` with `mapAckIndexer{1:12, 2:5, 3:6}` — returns `6` at `GoFuncSpec` strength, restated with the declarative spec (`IsCommittedIndex [1,2,3] ackedThreeAll`). What n = 1 could not test and this does: the map range has `3! = 6` iteration orders and NONE is enumerated (one generic iteration + the permutation invariant); `slices.Sort` does real work over an array whose contents are known only as a MULTISET, collapsed by `mergeSort_eq_of_perm`; and `pos = n - (n/2+1)` lands at a nonzero index (1), so the readout is the middle element, not the only one. First-order readout `quorumThreeAllReturnsSix` + run-conditioned negative twin `quorumThreeAllNotTwelve` (at `12`, the largest acked index). **Owed from this rung:** ~~the `make([]uint64, n)` fit branch, the general-`n` sort/readout, and the ∀-config theorem~~ — **ALL PAID 2026-08-01 (phase 4), row below.** |
| `quorum/committed-index-real` + `/three-all` (same pin) | `GoLean.Surface.committedIndexAllConfigs` | **THE ARC GOAL, PAID (2026-08-01, proof-automation arc phase 4) — `CommittedIndex` for EVERY CONFIG.** For every voter list `c`, every acked map `acked : Nat → Option Nat` and every heap snapshot pair ENCODING them (`EncodesConfig`/`EncodesAcked`), the pinned lowering of the real `main.MajorityConfig.CommittedIndex` — called on a heap-carried receiver and a heap-carried `AckedIndexer`, from the caller's environment — delivers a value satisfying the DECLARATIVE quorum spec `IsCommittedIndex c acked`, at `GoSpec` strength. What the concrete rungs could not test and this does: BOTH branches of `if len(stk) >= n` (the on-stack reslice AND `make([]uint64, n)`, with the scratch array's address and capacity existential); voters that never reported (`AckedIndex` → `(0, false)`: no write, no decrement, zeros in the LOW slots — the `ackedOrZero` semantics, which every concrete rung's total acked map hid); `slices.Sort` at a SYMBOLIC length, computed by induction over the machine's own `for i in [:len]` loops (`applyStmtOp_sortSlice_ints`) instead of unrolling; the `pos = n - (n/2+1)` readout at a symbolic index; and the map-entry SEARCH at a symbolic entry array (`mapLookupValue_hit`/`_miss`). First-order readout `committedIndexAllReturnsSix` (an INSTANCE of the theorem at the 3-voter encoding, also its non-vacuity witness) + run-conditioned negative twin `committedIndexAllNotTwelve` + the ∀-quantified refutation `committedIndexAll_refutes_wrong`. **Statement correction, recorded:** the target gained a `c.length < 2 ^ 63` REPRESENTABILITY hypothesis — without it the statement is FALSE, not unproven (at `2^63` voters `n := len(c)` wraps negative in Go's `int` and `stk[:n]` panics). |
