...
- `recut --remap`: **~4 min CPU (253–246 s user across runs), 0 files
...
- The proposals *classify* perfectly: the 117 segments proposed at
  **100 %** are exactly the unchanged ones, and the 11 sub-100 scores
  are exactly the changed sections. But the changed-section scores
  span **69–99 %** (`@Type_parameter_declarations` 69 %,
  `@Composite_literals` 85 %, `@Close` 92 % — exactly the
  close-builtin change), and one genuinely changed section
  (`@Allocation`) got **no candidate at all** at the 0.40 default —
  a fingerprint false negative. All of that signal is then discarded:
...
1. **Anchoring diff.** Before (or instead of) the LCS: match lines
   ...
   it sits (this pair has 31 hunks; the gaps around them are all tiny).
...
3. **`recut --remap --apply [--threshold <f>]`** — with the partition
   ...
   this pilot's 128 would heal, leaving the 11 changed + 1 no-candidate
   for review, which is the correct residue). Named segments keep
