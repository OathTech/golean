...
All numbers at the current tracked baseline (3260 cases, 3066 PASS /
194 FAIL — the round-8b merge-train UNION 2026-09-03 [AGENT] of the
...
base — 61 born rows under `stdlib-source/` (55 PASS / 6 FAIL, the born
ids disjoint from every main row), 6 FAIL→PASS flip(s), 0 PASS→FAIL
flip(s) — = 3260 / 3066 / 194: 3199 + 61 = 3260; 3005 + 55 + 6 − 0 = 3066;
194 + 6 − 6 + 0 = 194; re-derived from the union baseline's data rows,
not summed by hand (the lane's own figure at this commit was 3256 / 3062 / 194).
The two records, in merge order: (i) the lane's own — (3256 cases, 3062 PASS /
194 FAIL — the `stdlib-source-1` re-pin 2026-09-03 [AGENT] as amended at
its audit fix round, stdlib source-through slice 1: the previous pin's
3195 / 3001 / 194 with six shim-surface FAIL/frontend-export rows flipped
PASS by the real library source — `strings/split-conformance/empty-sep`, `strings/shim-value-refused/{shimmed-value,unmodeled-value}`,
`strconv/format-parse/unmodeled-member`, `panic-recover/shim-refusal-unrecoverable/{split-recover,render-recover}`
— plus 61 born rows under `stdlib-source/` (55 PASS + 6 FAIL by design
on FR-21's Cases): 3195 + 61 = 3256; 3001 + 6 + 55 = 3062; 194 − 6 + 6
= 194 — re-derived from the data rows. (ii) main's chain — (3199 cases, 3005 PASS /
