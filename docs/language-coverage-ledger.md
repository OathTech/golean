...
| 25 | unlowerable BASIC type in a signature / interface requirement list — the opaque-basic marker (FR-23's mode widened; [USER]-approved rider «(3) yes, makes sense», relayed) | FR-25 | 5 | S (this lane, checkpoint B) |
...
All numbers at the current tracked baseline (3376 cases, 3168 PASS /
208 FAIL — the FR-24 checkpoint re-pin 2026-09-04 [AGENT], lane `fr24`
on main aceb0dcb, §8k): main's 3365 / 3165 / 200 + 11 born rows (3 PASS
/ 8 FAIL/frontend-export, red BY DESIGN on FR-24 / FR-25 / FR-26 / FR-14)
+ 0 FAIL→PASS flips − 0 PASS→FAIL: 3365 + 11 = 3376; 3165 + 3 = 3168;
200 + 8 = 208 — re-derived from the baseline's data rows, not summed by
hand (the baseline header carries the same `awk` tally). The paragraphs
