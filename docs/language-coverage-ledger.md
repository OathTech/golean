...
| Receive_operator | covered(A) | `channels/recv-*` (edge, stmt, map-elem, order) + comma-ok matrix + closed/nil semantics; BUG-023/026/057 arcs all landed here; SHORT-CIRCUIT-RHS receives → FR-2, owner mini-slice A6 (pinned red: `channels/recv-short-circuit/{and-rhs,or-rhs}` + the BUG-032 refusal family — the A claim is scoped outside that position; carve-out added at the audit fix round, reviewer C F2). |
