...
All numbers at the current tracked baseline (3347 cases, 3157 PASS /
190 FAIL — the round-10 merge-train UNION 2026-09-04 [AGENT], under the
[USER] merge sign-off «go ahead and merge these» 2026-09-04: `stdlib-source-2`
(its records lettered §8g on the lane, §8i here — `q-trylock`'s §8g and
`cedar-census`'s §8h landed first) replayed over main 1b8401c0 = 221d8964 +
round 9 (`bug087-paniktext`, `q-trylock`, `cedar-census`)): main's 3284 /
3085 / 199 + 63 born rows under `stdlib-source/` (60 PASS / 3 FAIL) + 12
FAIL→PASS flips − 0 PASS→FAIL: 3284 + 63 = 3347; 3085 + 60 + 12 = 3157;
199 + 3 − 12 = 190 — re-derived from the baseline's data rows, not summed
by hand; the two sides' row deltas are disjoint (checked by id). The
paragraphs below are the previous figures, kept as history:

The `stdlib-source-2` figure over main 221d8964, kept as history: 3323
cases, 3130 PASS / 193 FAIL — the `stdlib-source-2` re-pin 2026-09-03
[AGENT], §8i below: the previous pin's 3260 / 3058 / 202 + 63 born rows
under `stdlib-source/` (60 PASS / 3 FAIL) + 12 FAIL→PASS flips − 0
PASS→FAIL: 3260 + 63 = 3323; 3058 + 60 + 12 = 3130; 202 + 3 − 12 = 193 —
re-derived from the baseline's data rows.
...
1<<24 bound is gone; a 30 s runner-budget red; BUG-073; enters
untriaged-ids as `coverage`, ceiling 10 → 11); NO PASS→non-PASS.
After: 3323 rows (3130 PASS / 193 FAIL). Buckets: frontier 100 → 102,
post-vintage 74 → 63, others unchanged; total 202 → 193.
