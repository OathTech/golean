...
| FR-24 | a REACHED source-through library package-level VARIABLE whose TYPE does not lower kills the WHOLE export at `collectGlobals` — `encoding/binary.Write` (reached from cedar-go `types/record.go:35`) → `dataSize` → `var structSize sync.Map` → `sync.Map (only Mutex/RWMutex/WaitGroup/Once are modeled)`. Measured 2026-09-04 as the kill that replaces FR-22's on cedar-go (census addendum §9.2: 29/34 cases; the register's `encoding/binary` row said "an unreached sync.Map" — true only while nothing reaches `Write/Read/Size`). | emit.go `collectGlobals` — since the audit fix round (item 3) the refusal NAMES the var: `package-level var encoding/binary.structSize: its TYPE does not lower (sync.Map (only Mutex/RWMutex/WaitGroup/Once are modeled)) — FR-24: a reached var of an unlowerable type refuses the whole export …` (was the bare `sync.Map (only …)` text the census measured) | spec#Package_initialization (a zero-valued var with no initializer; nothing to skip), memo `docs/2026-09-03_stdlib-boundary-design.md` §2.1 (source-through reachability) | 1 (`init/library-var-type-unlowerable`, born-FAIL; subject never touches the var) | frontend, S: apply the FR-22 mechanism to an unlowerable TYPE — on an `unsupported` from `emitType` in `collectGlobals`, seed the cell as `$poisoned` and arm `quarantinedGlobals[v]` with the type refusal, so every reader (`dataSize`, hence `Write/Read/Size`) quarantines by name and the export survives; decide whether the same applies to USER-package vars (`var cache sync.Map` in main — same argument; flips any row red only on that text) and record it | seq → queue 24 (S, before FR-4 by size) — logged 2026-09-04, [USER] direction 3 (coordinator relay) |
...
| 24 | reached library var whose TYPE does not lower — `$poisoned` cell + reader quarantine (FR-22's mechanism; cedar-go's next kill: `binary.Write` → `structSize sync.Map`) | FR-24 | 1 | S (½ day) |
...
All numbers at the current tracked baseline (3365 cases, 3165 PASS /
200 FAIL — the FR-22/FR-23 slice re-pin 2026-09-04 [AGENT], lane
`fr22-fr23` on main 415f959a (= the round-10 union), §8j): main's 3350 /
3157 / 193 + 15 born rows (6 PASS / 9 FAIL/frontend-export, red BY DESIGN
on FR-22 / FR-23 / FR-24) + 2 FAIL→PASS flips (the two census witnesses)
− 0 PASS→FAIL: 3350 + 15 = 3365; 3157 + 6 + 2 = 3165; 193 + 9 − 2 = 200 —
