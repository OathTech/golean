...
All numbers at the current tracked baseline (3281 cases, 3084 PASS /
197 FAIL — the `q-trylock` re-pin 2026-09-03 [AGENT], §8g, PROVISIONAL at
the round-9 rebase onto main 312a4458 (`bug087-paniktext` merged) 2026-09-04
[AGENT]: the bug087 pin's 3264 / 3065 / 199 + 17 born-PASS rows + 2
FAIL→PASS flips, re-derived from the data rows, not summed by hand. The
paragraph below is the previous figure, kept as history:
...
*(Movement at the 2026-09-03 Q-TRYLOCK re-pin (lane `q-trylock`, §8g; PROVISIONAL at the rebase onto main 221d8964, re-derived at the fix round): frontier 100 → 99 — FR-5's `sync/promoted-mutex/trylock-expr`
flipped green (FR-5 retired); Q-* 10 → 9 — Q-TRYLOCK's
`sync/out-of-scope-trylock/trylock-uncontended` flipped green (the row
resolved); the slice's 17 new rows are born PASS and appear in no bucket.
...
### 8g. Movement at the Q-TRYLOCK slice (2026-09-03, lane `q-trylock`)

Baseline before: 3199 rows (3005 PASS / 194 FAIL, the hygiene-b1-stamps
F2 re-pin). This slice's full `ci --diff` (evidence dir
`docs/evidence/2026-09-03_q-trylock/`, `baseline-drift.txt`): +17 rows
ALL born PASS — `sync/trylock/*` 12 (4 strict at held states:
`locked-false`, `trylock-false-then-lock`, `rw-trylock-reader-held`,
`rw-try-writer-held`; 8 membership: 7 over {acquired, spurious false},
each `enumerated=2 exhibited=1 unexhibited=1`, plus
`spin-until-trylock` under `nonterm=` with members {42} and 4242
counted nonterm branches), `race/negative-sync/{overwrite-vs-trylock,
failed-trylock-no-edge}` racy (every path refuses; gc -race red 20/20),
`race/free-sync/{trylock-publish,rw-trylock-publish}` confluent and
`rw-tryrlock-acquire` membership {0, 8}; 2 FAIL→PASS flips — the
frontier reds `sync/out-of-scope-trylock/trylock-uncontended` (Q-TRYLOCK's
1) and `sync/promoted-mutex/trylock-expr` (FR-5's 1), both
frontend-export → membership PASS (a strict pin being impossible for a
width-2 draw); NO PASS→non-PASS flip; NO other row moved. After: 3216
rows (3005 + 17 + 2 = 3024 PASS / 194 − 2 = 192 FAIL ✓). Red buckets:
Q-* −1 (Q-TRYLOCK resolved, its §6 red column now 0), frontier −1
(FR-5 retired); the untriaged `coverage` class is unchanged (both reds
were frontend-export, never in it). The latitude inventory gains C12
(its (a) count 9 → 10) and the §0 site table its `tryLock` row; the
memory-model `locks` row stays covered(B) with TryLock now inside it.
