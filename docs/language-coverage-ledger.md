...
| FR-27 | EXPLICIT instantiation of a package-QUALIFIED source generic function — `mapset.Immutable[EntityUID](args...)` (cedar-go `types/entity_uid.go:143`, measured at the fr4-rowm census 2026-09-04 once FR-4 stopped killing the export): the callee/value is an `IndexExpr`/`IndexListExpr` whose base is a `SelectorExpr`; `genericFuncValue` resolves an identifier base only, so the arm refuses `generic instantiation <pos>` (a text that names a position, not the shape — and an absolute path at that). The same generic called with INFERRED arguments (`mapset.Immutable(args...)`) lowers through `emitQualifiedCall`. | emit.go emitExpr `*ast.IndexExpr` / `*ast.IndexListExpr` arms — `unsup("generic instantiation %s", e.fset.Position(ex.Pos()))` | spec#Instantiations, spec#Calls | 0 in the corpus (cedar-go census §11: one declaration, cedargo's NewEntityUIDSet; internal/eval's doInEval is the same shape) — a corpus row is owed with the fix | frontend, S: teach `genericFuncValue` (and the call path) a `SelectorExpr` base resolving to a source-package generic `*types.Func` → `funcInstanceAt(sel.Sel, fn)` (the Instances record is keyed by the selector's identifier); rename the refusal to name the shape | seq → queue 27 (S) — logged 2026-09-04 [AGENT], lane `fr4-rowm` |
| FR-28 | `len`/`cap` hoist between two potentially-panicking operands with a later ordered event in the same statement — BUG-062's RECORDED fail-closed residual (§2 Order_of_evaluation / Length_and_capacity: "one fail-closed refusal residual"), given a frontier row so `tools/lowerdiag` classifies it by name instead of the `expression-shape` catch-all (measured on cedar-go 2026-09-04: `x/exp/schema/internal/parser.lexer.skipWhitespaceAndComments`, the `drv-validate` driver's first stub). | emit.go the `len`/`cap` hoist arm — `unsup("%s of a potentially-panicking operand between a potentially-panicking operand to its left and a later ordered call/receive in the same statement …")` | spec#Order_of_evaluation, spec#Length_and_capacity | the BUG-062 red-by-design pins (BUGS.md BUG-062 Cases line) | frontend, M–L: the full-statement linearization (BUG-032) that realizes gc's left-to-right panic point; until then the refusal stands (a visible red, never a guessed order) | seq → queue 28 (M–L; a Tier-1 order-of-evaluation item, sequenced with FR-2/A6) — logged 2026-09-04 [AGENT], lane `fr4-rowm` |
...
| 27 | explicit instantiation of a qualified source generic (`pkg.F[T](x)`) — SelectorExpr base in genericFuncValue | FR-27 | 0 (+1 owed) | S |
| 28 | len/cap hoist between panicking operands (BUG-062 residual → BUG-032 linearization) | FR-28 | BUG-062 pins | M–L |
