...
All numbers at the current tracked baseline (3400 cases, 3189 PASS /
211 FAIL — the FR-4 / row-M re-pin 2026-09-04 [AGENT], lane `fr4-rowm`
on main 9128e6c0, §8n): checkpoint C's 3382 / 3172 / 210 + 18 born rows
(13 PASS / 5 FAIL/frontend-export, red BY DESIGN on FR-4 / FR-23 / the
(c)-pin C6) + 4 FAIL→PASS flips (`generics/stencil-quarantine/sibling`,
`slices/slices-sort-non-integer-refusal`, `stdlib-source/sort-op-shapes/
{defer-sort,go-sort}`) − 0 PASS→FAIL: 3382 + 18 = 3400; 3172 + 13 + 4 =
3189; 210 + 5 − 4 = 211 — re-derived from the baseline's data rows, not
...
*(Movement at the FR-4 / row-M slice 2026-09-04 (lane `fr4-rowm`, §8n; post-slice figure 3400 = 3189 / 211 — main 9128e6c0 was 3382 = 3172 / 210): frontier 124 → 124 — the FR-4 witness `generics/stencil-quarantine/sibling` and the three row-M reds `slices/slices-sort-non-integer-refusal`, `stdlib-source/sort-op-shapes/{defer-sort,go-sort}` flipped GREEN (−4), and 4 born-FAIL rows red BY DESIGN entered (+4): 3 on FR-4 (`generics/stencil-quarantine/{stencil-call-refuses,iface-dispatch,transitive-call}` — the stub refuses by name) and 1 on FR-23 (`generics/stencil-quarantine-iterseq/call`); (c)-pins 9 → 10 — `generics/stencil-quarantine/local-type-c6` entered on the ratified C6 pin (+1); FR-27 and FR-28 were born with 0 corpus reds (FR-28's are BUG-062's standing pins, already counted post-vintage). Q-*, (a), post-vintage unchanged: 124 + 9 + 11 + 8 + 59 = 211.)*
...
BORN FAIL/frontend-export, red BY DESIGN (frontier +4, (c)-pins +1):
  ...
  generics/stencil-quarantine-iterseq/call         FR-23 (the iter.Seq value in the caller; FR-12 behind it)
...
3382 + 18 = 3400 rows = 3189 PASS / 211 FAIL (3172 + 13 + 4 / 210 + 5 − 4);
frontier 124 − 4 + 4 = 124, (c)-pins 10 → 11 (incl. the unsafe marker); the
§8 closing arithmetic re-derived by row in the bucket table above (124 + 9
+ 11 + 8 + 59 = 211). 0 PASS→non-PASS (re-pin guard clean). Twin wire pin
