...
> **Fixed 2026-09-03** (lane `bug087-paniktext`, [USER] ruling «demonic choice so both are admitted», relayed): the machine admits both texts as a width-2 demonic choice (`ChoiceSite.nilValueMethodText`; latitude inventory R9a); the three Cases rows below plus `iface-dispatch-value-nil` and `mk-helper-value-nil` are membership rows (`members=2`). Evidence: `docs/evidence/2026-09-03_bug087-paniktext/`. The analysis below is the discovery record and stands as written.
...
- Post-landing correction (2026-09-04, lane `flaky-panic-wait` [AGENT]):
  `goroutines/goroutine-panic-under-wait` was landed here in the STRICT
  lane with `expected_status=panic`, but gc's outcome is
  schedule-dependent — the deferred `wg.Done()` runs during the child's
  unwind and wakes main, which can print and exit 0 before the crash
  lands (the round-13 merge train saw `ok 1` once under gate load). The
  machine's set is {panic "under-wait", ok 1} (dedup closes at 175
  nodes), so the row was re-laned to `membership members=2
  statuses=ok+panic` and a deterministic no-Done twin
  (`goroutine-panic-under-wait-no-done`, gc 20/20 panic plain and -race)
  was added; evidence `docs/evidence/2026-09-04_flaky-panic-wait/`.
  Lesson recorded in `docs/operational-lessons.md` (coin-flip-gate
  addendum).
