...
| T12 | Sub-step interleaving granularity + operational race-UB by construction (two-step na accesses, `naMode Reading n | Writing`; racy access → stuck) | lang.v:543-546,1352-1396@43d4efa | We rejected naMode (grows by revision, D2+D3); registry-granularity + the NPDRF reduction, SETTLED at channel-logic S4 (docs/2026-08-11_npdrf-reduction.md): the draft statement is refuted by theorem (`NPDRFReduction_refuted`), the corrected class-level `NPDRFClassReduction` is the open target (proved for never-spawning pools only), and StepM = StepMFine ∩ boundary-switch is machine-checked | **DEL** with an honest open metatheory debt on our side — their granularity story is complete by construction; ours rests on the unproven reduction |
