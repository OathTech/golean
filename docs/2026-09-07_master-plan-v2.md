...
| W4 | **Fix lane** (v1 §7.4 item (3), retained) | **FR-31/BUG-098 LANDED — round 29, 2026-09-09, main 56b4d5c5** (reviewed FIX-FIRST → fixed → delta MERGE-CLEAN). Previously: FR-31/BUG-098 DONE on branch, awaiting review (2026-09-09 [AGENT]; I1 member identity + matching + distinct promoted targets, three reds green, fresh Cedar census); BUG-104 `safeExpr`-style decomposition; the decoder `.getD .int` fallback (`NativeToIR.lean:1507,1584`); F8's certificate fingerprint | the 3 + 5 rows PASS; the BUG-098 guard retires; cedar `all` revives; stale certification is release-blocking |
