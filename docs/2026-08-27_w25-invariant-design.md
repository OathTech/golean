...
STATUS (2026-08-27, amended at the triage landing — read this whole
block before the note): **ADOPTED, BUT ITS OBJECT IS ARCHIVED.**

The note was ADOPTED as the W3 postcondition contract by [AGENT]
adjudication under the active autonomous directive (the goal monitor
refused a stop at the plan's named [USER] gate; conflict resolution
logged in `docs/raft-campaign-log.md`). The three §3 open points were
resolved as recommended (ghost-acks; separate lens readers; T1
scope).

**What the triage did to it (2026-08-27).** The artifact this note
designs — `proofs/GoLeanProofs/Specs/RaftPilot/Invariant.lean`, the
629-line clause contract — was **ARCHIVED rather than landed** at the
triage (an [AGENT] coordinator recommendation, ratified by [USER]
package assent at the 2026-08-27 sign-off; recoverable at
`archive/callspec-era`), including the F-1-defective
`ElectedAt.logBridge`/`commitTie` clause pair. The design below is
therefore a LIVE DESIGN WITH NO LIVE OBJECT: nothing in the tree
implements it today.

**The gate re-runs, and it is the user's.** The G-INV amendment
re-designs the clause inventory at the Iris tier; that re-design
carries the [USER] gate the original adjudication bypassed. In other
words the [AGENT] adjudication recorded above is *not* grandfathered
by the triage sign-off — the substance is still Mike's to approve,
amend, or redirect at G-INV, and the ghost-acks extension in
particular (which DID land, and is cited by the mechanism registry)
is the one piece whose adoption outran its gate. Treat §§1–3 as the
input to G-INV, not as settled contract.

This note lands on `main` at the triage landing as the cited
authority for the reclassification, the deletions, and the
`Ghost.acks` extension (pre-merge audit, semantics-dimension M1 /
claim-dimension F5: a cited authority that lives only in an
unmerged worktree is not an authority).
