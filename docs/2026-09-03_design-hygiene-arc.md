...
|---|---|---|---|---|
| 4 | B4 + C5 | `c-arc-b4` | branch-complete 2026-09-05: `40fd1903` (Signal), `165822ef` (Thread/abort/flag) + records; gate `ci --diff` at the C5 tip PASS at zero drift (tally in the evidence README); whole-corpus per-consumption trace byte-identical (23115 records) and status/obsHash identical on 20749 compared lines (20748 traced (row, stream) lines + the one tracer ERROR row, design note §5) at both commits; not merged; audit fix round 2026-09-05 (R1–R7, R10, tracer fix — records, three lemma additions, the enumerator error type) | design note `docs/2026-09-05_c-arc-b4-design.md`; evidence `docs/evidence/2026-09-05_c-arc-b4/`; `Config` 16 → 10 constructors, `Step` 141 → 108 rules, `StepM` 5 → 7 rule classes (`strip`, `abort`); OWED: `Park` type, the C3-time `Status`/frame-entry revisits; commit 1's own gate run was RED for one tooling reason (13 misplaced-linter warnings, fixed in commit 2) with the differential itself at zero drift — recorded, not hidden |
| C-arc C2 | well-founded `TypeEnv` (G-C2) | `c-arc-c2` | branch-complete 2026-09-05 (SHAs in the design note §9); gate `ci --diff` PASS at zero drift beyond the twin pin (a permutation of the 92 type entries); not merged | design note `docs/2026-09-05_c-arc-c2-design.md`; evidence `docs/evidence/2026-09-05_c-arc-c2/`; frontend order pass + self-check, decoder `Accepted` clause, `Ty.defined (idx)`, 14 fuel families + `typeResolutionFuel` + seal deleted; merge-protocol step 5a (`--slow` re-cert) applies at the train (wire.go/NativeToIR.lean touched); rebased onto main cf858f0f at merge train round 17 (composed with fr19-bug097 + c-arc-b4 — design note §9, round-17 paragraph; twin re-emitted f89e1c9e… → a9a2e2b1…; 3523 + 5 = 3528 = 3283 / 245) |
