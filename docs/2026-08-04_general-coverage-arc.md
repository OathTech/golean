...
PASS→FAIL at every one of the re-pins; pass count non-decreasing;
every new capability's cases classified correctly before its
implementation landed. (Numbers made exact at the arc-final audit, F18
2026-08-06 — the first draft said "~20 re-pins" and "strictly
monotone": there were **31** re-pins of `baselines/native-full.tsv`
over e8e7ef2..d7171d6, and four of them held the pass count flat
(627→627, 712→712, 733→733, 749→749 — guardrails-first/audit-response
commits that add red cases). The load-bearing claims were re-verified
id-level and hold: zero PASS→FAIL at all 31 re-pins, endpoints
603/873 → 837/966 exact, and all 14 arc-introduced reds named below.)
Every exit criterion below is met. The remaining 129 reds: every
ARC-INTRODUCED red (14 ids) is in the recorded non-target list —
channels 38+ (own arc), complex 20 (deferred, floats note §9),
range-over-func 9 (Go 1.23 iterators, pre-existing gap), the
pre-existing rune-conversion and tuple-assign backlog clusters,
BUG-005's 3 differential reds (deferred behind the membership lane by
plan), BUG-012/BUG-014 pins, and the designed-red envelope/deviation
pins (goto capture/address family, local-type-argument,
hidden-dep-order, to-int-out-of-range, quarantined-init deps) — but
the first draft's "exactly the recorded non-targets" overstated the
enumeration: ~25 PRE-EXISTING backlog reds fall outside the listed
clusters (arrays/pointer-array, bools evaluation-order ×3,
methods/value-auto-deref, pointers/nil-*-panic ×2, structs/tag-* ×3,
BUG-008's maps/imported-named-key-unhashable, BUG-010's
interfaces/imported-package-name-collision, BUG-004's four
panic-rendering pins, and the untriaged remainder tracked by
`baselines/untriaged-ids` — 15 ids with no BUGS.md entry, counted by
the check-4 ratchet). The arc neither caused nor hid them; "recorded"
for the untriaged 15 means only "counted by the ratchet". New bugs
filed during the arc: BUG-012 (bare-call discard), BUG-013 (fixed
in-arc), BUG-014 (defined-slice/map nil elements). Flagged for the arc-final audit: the four decided design
...
- Zero regressions throughout (the failing-set diff, per slice); pass
  count non-decreasing (originally "strictly monotone" — corrected with
